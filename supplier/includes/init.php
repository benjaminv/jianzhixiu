...
require(ROOT_PATH . '/includes/lib_odooerp.php');
/* 对用户传入的变量进行转义操作。*/
    ...
    if (!empty($_COOKIE['ECSCP']['supplier_id']) && !empty($_COOKIE['ECSCP']['supplier_user_id']) && !empty($_COOKIE['ECSCP']['supplier_pass']))
    {
        ...
        else
        {
            ...
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['supplier_pass'])
            {
				...
				/* xxxxxxxxxxxxx */
				$dm299com_supplier = $db->getRow("SELECT cut,extpintuan,lucky_buy FROM " . $ecs->table('supplier') . " WHERE supplier_id = '" . $row['supplier_id'] . "'");
				$_SESSION['dm299com_supplier_cut'] = $dm299com_supplier['cut']; // 店铺的id
				$_SESSION['dm299com_supplier_extpintuan'] = $dm299com_supplier['extpintuan']; // 店铺的id
				$_SESSION['dm299com_supplier_lucky_buy'] = $dm299com_supplier['lucky_buy']; // 店铺的id
