...
//	}
   ...
   //添加
    ...
    $refund_money_2 = $_REQUEST['refund_money_2'] + $_REQUEST['refund_shipping_fee_bf'];

//    if($_REQUEST['refund_type']=='4'){
        $order_id=$order['order_id'];
        $sql = "SELECT log_id,order_amount FROM " . $GLOBALS['ecs']->table('pay_log') ." WHERE order_id = '$order_id' and order_type=0 ";
        $row=$GLOBALS['db']->getRow($sql);
        $order_sql = "select pay_name from " . $GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
