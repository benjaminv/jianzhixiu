...
function sendSMS ($mobile,$param,$template_code='')
{	
/*
	...
	return ($cc['err_code']=='0') ? true : false;
*/
	if( $template_code=='' ) return false;

	include_once(ROOT_PATH ."aliyunyu/sms.php");
	$sms = new SmsDemo($GLOBALS['_CFG']['appkey'],$GLOBALS['_CFG']['secretKey']);
	$response = $sms->sendSms(
		$GLOBALS['_CFG']['dayu_sign'], // 短信签名
		$template_code,				  // 短信模板编号
		$mobile,					  // 短信接收者
		$param,	  					  // 短信模板中字段的值
		'123456');
	$resp= (array)$response;
	return ($resp['Code']=='OK') ? true : false;
