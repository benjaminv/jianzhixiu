...
class integrate
{
	...
	/* 邮箱是否验证 */
	public $field_email_validated = '';

	/* 手机是否验证 */
	public $field_mobile_validated = '';
	...
	function remove_user ($id)
	{
		...
		if($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop))
		{
			...
			if($col)
			{
				...
				$sql = "DELETE FROM " . $GLOBALS['ecs']->table('weixin_user') . " WHERE " . db_create_in($col, 'ecuid'); // 删除微信账号
				$GLOBALS['db']->query($sql);
