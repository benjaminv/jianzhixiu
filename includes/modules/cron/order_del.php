...
$cron['order_del_qq_120029121_day'] = !empty($cron['order_del_qq_120029121_day'])  ?  intval($cron['order_del_qq_120029121_day']) : 1 ;
...
$sql_www_ecshop68_com = "select order_id FROM " . $ecs->table('order_info') .
           " WHERE pay_status ='0' and add_time < '$deltime' AND `order_status` NOT IN (".OS_CANCELED.",".OS_INVALID.") ";
...
while ($row_www_ecshop68_com=$db->fetchRow($res_www_ecshop68_com))
{
  ...
  elseif ($cron['order_del_qq_120029121_action'] == 'remove')
  {
      ...
      {
         hange_order_goods_storage($row_www_ecshop68_com['order_id'], false, SDT_SHIP);
      }elseif ($_CFG['stock_dec_time'] == SDT_PLACE){
         change_order_goods_storage($row_www_ecshop68_com['order_id'], false, SDT_PLACE);
      }
}
