...
class deppon
{
    ...
    function calculate($goods_weight, $goods_amount, $goods_number)
    {
        ...
        $sql = 'SELECT IFNULL(ur.special_rank,0) FROM ' . $GLOBALS['ecs']->table('users') . " AS u left join ". $GLOBALS['ecs']->table('user_rank') ." AS ur on u.user_rank=ur.rank_id WHERE u.user_id = '". $_SESSION['user_id'] ."'";  //jx
        $special_rank = $GLOBALS['db']->getOne($sql);
        if (isset($_SESSION['flow_order']) && empty($_SESSION['flow_order']['bonus_id']) && $this->configure['free_money'] > 0 && $goods_amount >= $this->configure['free_money']&&$special_rank!=1)
        {
            return 0;
        }
        else
