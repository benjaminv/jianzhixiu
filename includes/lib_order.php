...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $order['formated_deductible_amount'] = price_format($order['deductible_amount'], false);
...
{
	if( !isset($consignee['province']) ) {
		$consignee=array('country' => 0,'province' => 0,'city' => 0,'district' => 0);
	}
    /* 初始化订单的扩展code */
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		foreach ($goods AS $val)
		{
			...
			if ( empty($total['supplier_shipping']) ){
					 $total['supplier_shipping'] = array(
						$row_supp['supplier_id'] =>array('goods_number'=>0)
					);

			}

			if( empty($total['supplier_goodsnumber'][$row_supp['supplier_id']]) ) {
				$total['supplier_goodsnumber'][$row_supp['supplier_id']] = 0;
			}
			...
			if ( empty($total['goods_price_supplier']) ){
				$total['goods_price_supplier']=array($row_supp['supplier_id']=>0);
			}
			if ( empty($total['goods_price_supplier'][$row_supp['supplier_id']]) ){
				$total['goods_price_supplier'][$row_supp['supplier_id']] =0;
			}
...
function get_order_bonus($order_amount,$user_id,$order_time,$supplier_id=0){
	 ...
	 //echo "<pre>";print_r(date('Y-m-d H:i:s','1558255841'));
    ...
    if (!empty($row)){
        if ($row['special_rank']){
            ...
            return $GLOBALS['db']->getRow(
                'SELECT discount, rank_id FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE special_rank=0 AND min_points >= '$rank_points' ORDER BY min_points ASC LIMIT 1"
            );
        }
    }else{
        return $GLOBALS['db']->getRow(
            'SELECT discount, rank_id FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE special_rank=0 AND '$rank_points' >= min_points ORDER BY min_points DESC LIMIT 1"
        );
    }
}
