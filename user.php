...
function action_login ()
{
	...
	if(empty($back_act))
	{
		if(empty($back_act) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
		{
			...
			{
				$http_referer = "http://".$GLOBALS['_SERVER']['HTTP_HOST'].'/index.php';
			}
			
			$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? "http://".$GLOBALS['_SERVER']['HTTP_HOST'].'/index.php' : $http_referer;
		}
		else
		{
			$back_act = "http://".$GLOBALS['_SERVER']['HTTP_HOST'].'/user.php';
		}
	}
...
//AXAJ 登陆修改
	...
	// 如果来自找回密码页面则跳转到首页
	if(strpos($back_act, $http_host) == false)
	{
		$back_act = "http://".$GLOBALS['_SERVER']['HTTP_HOST'].'/index.php';
	}
...
function action_add_booking ()
{
	...
	if($_GET['spec'] != '')
	{
		$goods_attr_id = intval($_GET['spec']);
...
function action_user_bonus ()
{
	...
	$result['no_have'] = intval($_GET['no_have']);
