...
if ($_REQUEST['act'] == 'add')
{
	...
	$sql = "SELECT rank_id, rank_name FROM " . $ecs->table('user_rank') . " WHERE 1";
...
if ($_REQUEST['act'] == 'insert')
{
	...
	$goods_use = $_POST['goods_use'] ? intval($_POST['goods_use']) : 0;
	$money_type = $_POST['money_type'] ? intval($_POST['money_type']) : 0;
	$use_goods_config = ($goods_use==1) ?  trim($_POST['goods_ids']) : "";
	$is_distribute = $_POST['is_distribute'] ? intval($_POST['is_distribute']) : 0;
	$money_block = array();
	if($_POST['money']['money_left']){
		foreach($_POST['money']['money_left'] as $k=>$v){
			$block = array(
				'left'=>$v,
				'right'=>$_POST['money']['money_right'][$k],
				'money'=>$_POST['money']['bonus_money'][$k],
			);
			$money_block[] = $block; 
		}
	}
	$money_block = ($money_type==1) ? serialize($money_block) : "";
	$user_rank_on = $_POST['user_rank_on'] ? intval($_POST['user_rank_on']) : 0;
	$user_rank = ($user_rank_on==1) ?  implode(',',trim($_POST['user_rank'])) : "";
	//echo "<pre>";print_r($money_block);exit;
    /* 插入数据库。 */
	$sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,bonus_code,user_bonus_max,min_amount,min_goods_amount,goods_use,use_goods_config,money_type,money_block,user_rank_on,user_rank,is_distribute)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
...
if ($_REQUEST['act'] == 'edit')
{
	...
	if($bonus_arr['use_goods_config']){
		$sql = 'SELECT goods_id,goods_name FROM '.$ecs->table('goods').' WHERE goods_id '.db_create_in($bonus_arr['use_goods_config']);
		$bonus_arr['dst_goods_lists'] = $db->getAll($sql);
	}
	if($bonus_arr['money_block']){
		$bonus_arr['money_block'] = unserialize($bonus_arr['money_block']);
	}
	if($bonus_arr['user_rank']){
		$bonus_arr['user_rank'] = explode(',',$bonus_arr['user_rank']);
	}
	//echo "<pre>";print_r($bonus_arr['money_block']);
    $smarty->assign('lang',        $_LANG);
...
//    }
	...
	$user_rank_on = $_POST['user_rank_on'] ? intval($_POST['user_rank_on']) : 0;
	$user_rank = ($user_rank_on==1) ?  implode(',',$_POST['user_rank']) : "";
	$money_block = ($money_type==1) ? serialize($money_block) : "";
    ...
    $sql = "UPDATE " .$ecs->table('bonus_type'). " SET ".
		   ...
		   "use_goods_config= '".$use_goods_config."', ".
		   "money_block		= '".$money_block."',".
		   "user_rank_on	= '".$user_rank_on."',".
		   "user_rank	= '".$user_rank."',".
    ...
    foreach ($user_list AS $key => $val)
    {
        ...
        if (add_to_maillist($val['user_name'], $val['email'], $tpl['template_subject'], $content, $tpl['is_html']))
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    ...
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_SUCCEED. ",'$bonus_type[type_money]')";
            $db->query($sql);
        }
        else
        {
...
if ($_REQUEST['act'] == 'send_by_print')
{
	...
	$bonus_info = bonus_type_info($bonus_typeid);
    ...
    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
        ...
        $db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn,bonus_money) VALUES('$bonus_typeid', '$bonus_sn','$bonus_money')");
...
elseif($_REQUEST['act'] == 'bonus_send'){
    /* 获取红包类型数据 */
    $type_id = !empty($_GET['bonus_type']) ? intval($_GET['bonus_type']) : 0;
    $bonus_arr = $db->getRow("SELECT type_id,type_name FROM " .$ecs->table('bonus_type'). " WHERE type_id = '$type_id'");
	
    $smarty->assign('bonus_arr',   $bonus_arr);
	$smarty->assign('form_act',   'bonus_send_operator');
	
    assign_query_info();
    $smarty->display('bonus_send.htm');
	exit;
}
elseif($_REQUEST['act'] == 'bonus_send_operator'){
	
	$send_type = $_POST['send_type'] ? intval($_POST['send_type']) : 0;
	...
	if($send_type == 0){
		...
		if(!isset($user_info) || empty($user_info)){
			 $link[] = array('text' => '会员不存在','href' => 'bonus.php?act=list');
            sys_msg('操作失败', 0, $link);
		}else{
			//按照会员发放红包
			$money = $_POST['money'] ? abs(floatval($_POST['money'])) : 0;
			if(send_bonus_by_userid($user_id,$money,$type_id)){
				admin_log('会员id'.$user_id, 'send_operator', 'bonus_send');
				$link[] = array('text' => '操作完成','href' => 'bonus.php?act=list');
				sys_msg('操作完成', 0, $link);
			}else{
				$link[] = array('text' => '操作失败','href' => 'bonus.php?act=list');
				sys_msg('操作失败', 0, $link);
			}
		}
	}else{
		//按照订单进行发放
		$order_sn = $_POST['order_sn'] ? trim($_POST['order_sn']) : "";
		$sql = "SELECT order_id FROM ".$ecs->table('order_info')." WHERE order_sn='".$order_sn."'";
		$order_info = $db->getRow($sql);
		if(!isset($order_info) || empty($order_info)){
			  $link[] = array('text' => '订单不存在','href' => 'bonus.php?act=list');
            sys_msg('操作失败', 0, $link);
		}else{
			//按照订单发放红包
			include_once(ROOT_PATH . 'includes/lib_order.php');
			send_order_bonus($order_info['order_id'],0);
			admin_log('订单号'.$order_sn, 'send_operator', 'bonus_send');
			$link[] = array('text' => '操作完成','href' => 'bonus.php?act=list');
            sys_msg('操作完成', 0, $link);
		}
	}
...
elseif($_REQUEST['act'] == 'exportlist'){
	...
	$list = get_bonus_list2();
	
	 printf_csv($list['item']);
    echo "<pre>";
  print_r($list);exit;
...
function printf_csv($result)
{
    ...
    $data='id'.','.'红包类型'.','.'会员id'.','.'金额'.','.'使用订单'.','.'使用时间'.','.'会员名称'. '
';
    foreach ($result as $key => $val)
    {
         $data .= $val['bonus_id'] . ','  . $val['type_name']  . ',' . $val['user_id'] . ',' . $val['bonus_money'] . ',' . $val['order_sn'] . ',' . $val['used_time'] . ',' . str_replace(array("/r/n", "/r", "/n"), '', $val['user_name']). ' 
';
    }
...
function send_bonus_by_userid($user_id,$money,$send_type){
	$bonus_type_id = $send_type;
	$data=array(
		'bonus_type_id'=>$send_type,
		'bonus_money'=>$money,
		'user_id'=>$user_id,
	...
	$res = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_bonus'), $data, 'INSERT', '', 'SILENT');
	return $res;
}
...
function get_bonus_list()
{
    ...
    $where = empty($filter['bonus_type']) ? ' WHERE 1 ' : " WHERE bonus_type_id='$filter[bonus_type]'";
	if(isset($filter['user_name']) && !empty($filter['user_name'])){
		$where .= " AND u.user_name like '%".$filter['user_name']."%' ";
	}
	$sql = "SELECT COUNT(ub.bonus_id) ".
