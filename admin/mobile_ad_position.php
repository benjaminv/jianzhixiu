...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['position_add'], 'href' => 'mobile_ad_position.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('mobile_ad_position_list.htm');
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href' => 'mobile_ad_position.php?act=list', 'text' => $_LANG['ad_position']));
    $smarty->assign('posit_arr',   array('position_style' =>'{foreach from=$ads item=ad}' ."\n". '{$ad}' ."\n". '{/foreach}'));

    assign_query_info();
    $smarty->display('mobile_ad_position_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("position_name", $position_name) == 0)
    {
        ...
        $link[1]['href'] = 'mobile_ad_position.php?act=add';

        $link[2]['text'] = $_LANG['back_position_list'];
        $link[2]['href'] = 'mobile_ad_position.php?act=list';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        ...
        if ($db->query($sql))
        {
           ...
           /* 提示信息 */
           $link[] = array('text' => $_LANG['back_position_list'], 'href' => 'mobile_ad_position.php?act=list');
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('mobile_ad_position_list.htm'), '',
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'mobile_ad_position.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
