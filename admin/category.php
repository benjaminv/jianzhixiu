...
if ($_REQUEST['act'] == 'insert')
{
 ...
 /* 代码增加_start Byjdy */
	...
	$cat['type_img']        =basename($image->upload_image($_FILES['type_img'], $catimg_dir));
	//$image->upload_file_oss(DATA_DIR ."/category_img/". $cat['type_img']); //新增加 by markzhou
   ...
   $cat['attr_qq120029121']  = !empty($_POST['attr_qq']) ? $_POST['attr_qq'] : '';
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav FROM ". $ecs->table('category') . " WHERE cat_id = '$cat_id'");
        ...
        // 分类图标
	if ($_FILES['type_img']['tmp_name'] != '')
	{
		if (!$image->check_img_type($_FILES['type_img']['type']))
		{
    ...
    if ($_FILES['type_img']['tmp_name'] != '' && $_FILES['type_img']['tmp_name'] != 'none' )
    {
		
		/* 删除原来的图片文件 */
            $sql = "SELECT type_img FROM " . $ecs->table('category') ." WHERE cat_id = '$_REQUEST[cat_id]'";
            $type_img = $db->getOne($sql);
			if ($type_img)
            {
                @unlink('../data/category_img/' . $type_img);
				
            }
			$type_img=basename($image->upload_image($_FILES['type_img'], $catimg_dir ));
