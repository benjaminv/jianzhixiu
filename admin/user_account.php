...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    $acc_type    = isset($_POST['acc_type'])    ? intval($_POST['acc_type'])        : 0;
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1')
        {
           ...
           // exit;
           // print_r(file_get_contents($autoUrl."mobile/weixin/weixin_remind.php?notice=10&is_one_user=".$account['user_id']."&money=".$fmt_amount));
            //print_r("ceshi");
            //exit;
            if($acc_type ==0)
            {
                //微信红包or企业转账支付用户提现金额
                $sql = "SELECT * FROM " .$ecs->table('weixin_user'). " WHERE ecuid = '$account[user_id]'";
                $ret = $db->getRow($sql);//获取提现用户微信资料

                if(!empty($ret['fake_id']))
                {//微信id不为空
                ...
                {
                    $link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
                    sys_msg("该用户不是授权登录用户，不能微信提现", 0, $link);
                }
            }
            else
            {

                require('../includes/AlipayAop/AlipayAop.class.php');
                $osn             =  'jzxalipay'.date("Ymd",time()).date("His",time()).rand(1111,9999);;
                $payee_account   = $account_list['alipay_cn'];
                $payee_real_name = $account_list['alipay_name'];
                $amount1          = $fmt_amount;
                $AlipayAopDb = new AlipayAop();
                $msg = $AlipayAopDb->AlipayFundTransToaccountTransfer($osn,$payee_account,$amount1,$payee_real_name);
                if($msg['success']=='1'){
                   ;
                }else{
                    sys_msg($msg['text'], 0, $link);
                    exit;
                }
            }
            ...
            tixian($account['user_id'],$fmt_amount);
            //更新会员余额数量
