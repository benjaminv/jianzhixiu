...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
	...
	if ($_REQUEST['act'] == 'list'&&isset($_REQUEST['supp']))
	{
		$smarty->assign('is_goods_list',1);
	}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 处理商品数据 */
    $points_payment_amount =  isset($_REQUEST['points_payment_amount']) ? intval($_REQUEST['points_payment_amount']) : 0;
    $shop_price = !empty($_POST['shop_price']) ? $_POST['shop_price'] : 0;

    // 积分购买金额不得超过商品售卖金额
    if ($points_payment_amount > $shop_price) {
        sys_msg("积分购买金额不得超过商品售卖金额");
    }
    ...
    $goods_unit = $_POST['goods_unit'] ? $_POST['goods_unit']:'件';
   //早市
    $xg_goods_number = isset($_POST['xg_goods_number']) ? $_POST['xg_goods_number'] : 0;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (xg_goods_number, goods_name,goods_unit, goods_name_style, ghost_count, goods_sn,morning_market_price,is_morning_market, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, suppliers_id, cost_price, points_payment_amount)" .
                "VALUES ('$xg_goods_number','$_POST[goods_name]','$goods_unit', '$goods_name_style', '$ghost_count', '$goods_sn','$morning_market_price','$is_morning_market', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$suppliers_id', '$cost_price','$points_payment_amount')";
				$is_distrib_goods = 1;
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (xg_goods_number, goods_name,goods_unit, goods_name_style, ghost_count, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                    "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date,goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, cost_price,points_payment_amount)" .
                "VALUES ('$xg_goods_number','$_POST[goods_name]','$goods_unit', '$goods_name_style', '$ghost_count', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral', '$cost_price','$points_payment_amount')";
			$is_distrib_goods = 1;
        }
    }
    else
    {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "is_best = '$is_best', " .
                "is_new = '$is_new', " .
                "xg_goods_number = '$xg_goods_number', " .
                "is_hot = '$is_hot', " .
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                ...
                else
                {
					...
					//查询将要删除的货品
					$sql = "SELECT product_id FROM " .$ecs->table('products'). " WHERE goods_id = '$goods_id' and (goods_attr = '".$info['goods_attr_id']."' or goods_attr like '%|".$info['goods_attr_id']."' or goods_attr like '".$info['goods_attr_id']."|%' or goods_attr like '%|".$info['goods_attr_id']."|%')";
					$product_unlink_info = $GLOBALS['db']->getAll($sql);
					if(is_array($product_unlink_info) && !empty($product_unlink_info)){
						$product_ids = array_column($product_unlink_info,'product_id');
						$sql = "DELETE FROM " .$ecs->table('products'). " WHERE product_id IN (".implode(",",$product_ids).")";
						$db->query($sql);
						if(ODOO_ERP){
							$odooErpObj = OdooErp::getInstance();
							//edit yhy 同步删除erp中货品
							$res = $odooErpObj->syncUnlinkProductByProductids($product_ids);
						}
					}
                    continue;
                }
   ...
   /*编辑商品分成金额成为分销商品*/
    ...
    /* 清空缓存 */
    clear_cache_files();
	/*配置开启odooerp同步 start 同步产品到erp edit by yhy 2018/12/6 星期四*/
	if(ODOO_ERP){
		$odooErpObj = OdooErp::getInstance();
		$res = $odooErpObj->syncGoodsToErpByGoodsid($goods_id);
	}
	/*end*/
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        if ($_POST['type'] == 'trash')
        {
			...
			if(ODOO_ERP){
				$odooErpObj = OdooErp::getInstance();
				$res = $odooErpObj->syncRestoreGoodsByGoodsids($goods_id);
			}
            /* 记录日志 */
        ...
        elseif ($_POST['type'] == 'move_to')
        {
			...
			if(ODOO_ERP){
				$odooErpObj = OdooErp::getInstance();
				$res = $odooErpObj->syncGoodsToErpByGoodsids($goods_id);
			}
        }
        ...
        elseif ($_POST['type'] == 'restore')
        {
			...
			if(ODOO_ERP){
				$odooErpObj = OdooErp::getInstance();
				$res = $odooErpObj->syncStoreGoodsByGoodsids($goods_id);
			}
            /* 记录日志 */
        ...
        elseif ($_POST['type'] == 'drop')
        {
			...
			if(ODOO_ERP){
				$odooErpObj = OdooErp::getInstance();
				$res = $odooErpObj->syncUnlinkGoodsByGoodsids($goods_id);
			}
            /* 记录日志 */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->edit("is_delete = 1", $goods_id))
    {
		...
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			$res = $odooErpObj->syncRestoreGoodsByGoodsid($goods_id);
		}
...
elseif ($_REQUEST['act'] == 'restore_goods')
{
	...
	if(ODOO_ERP){
		$odooErpObj = OdooErp::getInstance();
		$res = $odooErpObj->syncStoreGoodsByGoodsid($goods_id);
	}
    clear_cache_files();
    $goods_name = $exc->get_name($goods_id);
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
	...
	if(ODOO_ERP){
		$odooErpObj = OdooErp::getInstance();
		$res = $odooErpObj->syncUnlinkGoodsByGoodsid($goods_id);
	}
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    $smarty->assign('attribute_count_3',        ($attribute_count + 4));
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    if ($result)
    {
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			//同步到erp系统 yhy
			$res = $odooErpObj->syncUnlinkProductByProductid($product_id);
		}
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'edit_product_sn')
{
    ...
    if ($result)
    {
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			//同步到erp系统 yhy
			$res = $odooErpObj->syncProductToErpByProductid($product_id);
		}
        clear_cache_files();
...
//-- 修改货品库存
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_xg_goods_number')
{
    check_authz_json('goods_manage');
    ...
    /* 修改货品库存 */
    $sql = "UPDATE " . $ecs->table('products') . " SET xg_goods_number = '$product_number' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    $product['xg_goods_number']  = $_POST['xg_goods_number'];
    /* 是否存在商品id */
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('products') . " (goods_id, goods_attr, product_sn, product_number, xg_goods_number)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "', '" . $product['xg_goods_number'][$key] . "')";
	...
	if(ODOO_ERP){
		$odooErpObj = OdooErp::getInstance();
		//同步到erp系统 yhy
		$res = $odooErpObj->syncProductToErpByProductids($product_ids);
	}
    clear_cache_files();
        ...
        if (!empty($product_array))
        {
            ...
            if ($db->query($sql))
            {
				if(ODOO_ERP){
					$odooErpObj = OdooErp::getInstance();
					//同步到erp系统 yhy
					$res = $odooErpObj->syncProductToErpByProductids($_POST['checkboxes']);
				}
                //记录日志
...
elseif($_REQUEST['act'] == 'sysnc_erp'){
	...
	if(!$goodsid){
		...
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			$res = $odooErpObj->syncGoodsAndProductToErpByGoodsid($goodsid);
			if(isset($res['SuccessCode']) && $res['SuccessCode'] == 1){
				if(isset($res['pres']['SuccessCode']) && $res['pres']['SuccessCode'] == 1){
					sys_msg("同步成功", 1, $link);exit;
				}else{
					sys_msg("商品同步成功，货品同步失败:".$res['pres']['faultString'], 0, $links);exit;
				}
			}else{
...
}elseif($_REQUEST['act'] == 'add_comment'||$_REQUEST['act'] == 'add_comment_handle'){//添加评论
	include_once(ROOT_PATH . '/includes/Pinyin.php');

	$is_add = $_REQUEST['act'] == 'add_comment'; // 添加还是编辑的标识
	$code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
	$code=$code=='add_comment' ? 'add_comment': '';
	if ($code == 'add_comment')
	{
		admin_priv('add_comment'); // 检查权限
	}
	else
	{
		admin_priv('goods_manage'); // 检查权限
	...
	if (!$is_add)
	{
		...
		$user_virtual_name = $post['user_virtual_name'];
		if($user_id>0){
			$user_type = $post['user_type'];
		}else{
			$user_type = 1;
			if(empty($user_name)&&!empty($user_virtual_name)){
				$time=time();
				$sql = "INSERT INTO " . $ecs->table('users_virtual') . "(user_name, reg_time,real_name)" . "VALUES ('$user_virtual_name', '$time','$user_virtual_name')";
				$db->query($sql);
		...
		if($user_type==0){
			...
			$user_info = $db->getRow("SELECT * FROM " . $ecs->table('users_virtual') . " WHERE user_id = '$user_id'");
		}
		$comment_type = 0;
		$id_value = $post['goods_id'];
		$email = $user_info['email'];
		$user_name = $user_info['user_name'];
		$content = $post['content'];
		$comment_rank = $post['rank'];
		...
		$add_time = strtotime($post['rank_time']);
		$ip_address = real_ip();
		$status = $post['is_show'];
		$rec_id = $post['rec_id'];

		/* 自定义标签 */
		$tags = ($post['comment_tag']) ? explode(",", $post['comment_tag']) : array();
		// 代码增加o_id
		$sql = "INSERT INTO " . $ecs->table('comment') . "(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address, user_id, status, rec_id, comment_tag, buy_time, hide_username, order_id,user_type)" . "VALUES ('$comment_type', '$id_value', '$email', '$user_name', '$content', '$comment_rank', '$add_time', '$ip_address', '$user_id', '$status', '$rec_id', '$comment_tag', '$buy_time', '$hide_username', '$o_id','$user_type')";
		$db->query($sql);

		// 代码增加
		clear_cache_files();

		echo "<script>alert('添加成功');self.location='comment_manage.php?act=list';</script>";
		exit();

	}
	/*查询出所有用户*/
	//$sql = "SELECT * FROM". $ecs->table('users') ."WHERE 1=1";
	//$user =$db->getAll($sql);
	$sql = "SELECT * FROM". $ecs->table('users_virtual') ."WHERE 1=1";
	$user_virtual =$db->getAll($sql);
	//foreach ($user as $k=>$v){
		//$user[$k]['type']=0;
		//$user[$k]['type_name']='真实用户';
	//}
	//foreach ($user_virtual as $k=>$v){
		//$v['type']=1;
		//$v['type_name']='水军';
		//$user[]=$v;
	//}
	/* 商品列表 */
	$sql = "SELECT * FROM " . $ecs->table('goods') . " WHERE goods_id > 0";
	$goods_list = $db->getAll($sql);
	/* 模板赋值 */
	$smarty->assign('code',    $code);
	$smarty->assign('ur_here', $_LANG['add_comment']);
	$smarty->assign('action_link', list_link($is_add, $code));
	$smarty->assign('user', $user_virtual);
	$smarty->assign('goods_list', $goods_list);
	$smarty->assign('form_act', $is_add ? 'add_comment' : ($_REQUEST['act'] == 'add_comment' ? 'add_comment' : 'add_comment_handle'));
	/* 显示商品信息页面 */
	assign_query_info();
	$smarty->display('goods_add_comment.htm');
}elseif($_REQUEST['act'] == 'ajax'&&$_REQUEST['type'] == 'goods_search'){//添加评论
	$wh=$_REQUEST['name'];
	/*查询出所有用户*/
	$sql = "SELECT * FROM". $ecs->table('goods') ."WHERE 1=1 and is_on_sale=1 and (goods_id = '$wh' or goods_sn  like '%$wh%' or goods_name  like '%$wh%')";
	$goods_list =$db->getAll($sql);
	ajaxReturn($goods_list);
}elseif($_REQUEST['act'] == 'ajax'&&$_REQUEST['type'] == 'check_user'){//添加评论
	$user_name=$_REQUEST['user_name'];
	/*查询出所有用户*/
	if(!empty($user_name)){
		$sql = "SELECT * FROM". $ecs->table('users_virtual') ."WHERE user_name='$user_name'";
		$user_list =$db->getAll($sql);
		if(!empty($user_list)){
			ajaxReturn(array('code'=>-1,'msg'=>'改用户名已被使用'));
		}
	}
