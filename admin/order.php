...
///订单导出
elseif ($_REQUEST['act'] == 'exportlist')
{
    $order_list = order_list1();
    printf_csv($order_list['orders']);
    echo "<pre>";
    print_r($order_list);
    exit;
}
...
else if ($_REQUEST['act'] == 'sync_erp'){
	$order_sn = $_REQUEST['order_sn'];
	$sql = "SELECT a.order_id,a.sync_erp as erp,a.order_sn,a.supplier_id,a.user_id,a.add_time,b.sync_erp,b.erp_id,a.consignee,a.address,a.mobile FROM ". $GLOBALS['ecs']->table('order_info') ."  a LEFT JOIN ". $GLOBALS['ecs']->table('supplier') ." b ON a.supplier_id=b.supplier_id  WHERE  a.order_sn = '".$order_sn."'";
	$orderInfo = $GLOBALS['db']->getRow($sql);
	
	if(!isset($orderInfo) || empty($orderInfo)){
		/* 操作成功 */
		$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
		sys_msg("订单不存在", 0, $links);exit;
	}

	if($orderInfo['supplier_id']>0 && ($orderInfo['sync_erp'] == 0 || $orderInfo['erp_id'] == 0) ){
		/* 操作成功 */
		$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
		sys_msg("商城没有配置erp", 0, $links);exit;
	}

	if($orderInfo['erp'] == 1){
		/* 操作成功 */
		$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
		sys_msg("订单已同步", 0, $links);exit;
	}
	//echo 111;exit;
	if(ODOO_ERP){
			//edit yhy 同步订单
			$odooErpObj = OdooErp::getInstance();
			$res = $odooErpObj->syncOrderByOrdersns(array($order_sn));
			...
			if(isset($res['SuccessCode']) && $res['SuccessCode'] == 1){
				...
				$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
				sys_msg("订单同步失败:".$res['faultString'], 0, $links);exit;
			}
	}else{
		$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
				sys_msg("系统对接erp未开启", 0, $links);exit;
	}
...
function return_user_surplus_integral_bonus($order)
{
    ...
    // 积分抵扣
    if ($order['user_id'] > 0 && $order['deductible_amount'] > 0) {
        log_account_change($order['user_id'], 0, 0, 0, $order['order_available_point'], sprintf($GLOBALS['_LANG']['return_order_integral'], $order['order_sn']));
    }
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		//秒杀订单*/
		$filter['skill_id'] = isset($_REQUEST['skill_id']) ? intval($_REQUEST['skill_id']) : 0;
        $filter['pre_sell_id'] = isset($_REQUEST['pre_sell_id']) ? intval($_REQUEST['pre_sell_id']) : 0;
	    ...
	    if ($filter['skill_id'])
        {
			...
			$where .= " AND o.extension_code =150 AND o.seckill_id = '$filter[skill_id]' ";
        }
        /* 众筹订单 */
        if ($filter['pre_sell_id'])
        {
            $where .= " AND o.extension_code = '" . PRE_SELL_CODE . "' AND o.extension_id = '$filter[pre_sell_id]' ";
        }
        ...
        /* 查询 */
        if($filter['supp']){
        	$sql = "SELECT o.order_id,o.sync_erp, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
...
function printf_csv($result)
{
   ...
   /* $data='订单号'.','.'下单会员'.','.'下单时间'.','.'收货人'.','.'联系电话'.','.'地址'.','.'商品总价'.','.'配送费'.','.'订单金额'.','.'余额金额'.','.'积分金额'.','.'红包金额'.','.'应付金额'.','.'付款状态'.','.'发货状态'.','.'支付方式'. '
';*/
    $data='订单号'.','.'下单时间'.','.'付款时间'.','.'发货单号'.','.'支付方式'.','.'配送方式'.','.'下单人'.','.'收件人'.','.'收货地址'.','.'收货电话'.','.'货号'.','.'商品名称'.','.'属性'.','.'价格'.','.'件数'.','.'小计'.','.'订单价格'. '
';
    foreach ($result as $key => $val)
    {
        foreach ($val['goods_list'] as $goods_key => $goods)
        {
            if((count($val['goods_list'])-1) == $goods_key)
            {
                $data .= 'L'.$val['order_sn'] . ','  . $val['formatted_add_time'] . ',' . $val['formatted_pay_time'] . ',' . $val['invoice_no'] . ',' . $val['pay_name'] . ',' . $val['shipping_name'] . ',' . $val['user_name'] . ',' . $val['consignee'] . ',' . $val['region'].$val['address'] . ',' . $val['tel'] . ',' . $goods['goods_sn'] . ',' . $goods['goods_name'] . ',' . str_replace(array("/r/n", "/r", "/n"), '', $goods['goods_attr']) . ',' . $goods['formated_goods_price'] . ','. $goods['goods_number'] . ','. $goods['formated_subtotal'] . ',' . $val['formated_total_fee'] . '
';
            }
            else
            {
                $data .= '' . ','  .  '' . ',' . '' . ',' .  '' . ',' .  '' . ',' .  '' . ',' .  '' . ',' .  '' . ',' .  '' . ',' .  '' . ',' . $goods['goods_sn'] . ',' . $goods['goods_name'] . ',' . str_replace(array("/r/n", "/r", "/n"), '', $goods['goods_attr']) . ',' . $goods['formated_goods_price'] . ','. $goods['goods_number'] . ','. $goods['formated_subtotal'] . ',' .  '' . '
';
            }
        }
    }
...
function order_list1()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        if($filter['supp']){
            $sql = "SELECT o.user_id,o.shipping_name, o.invoice_no, o.goods_amount,o.integral_money,o.shipping_fee,o.bonus,o.surplus,o.order_id,o.sync_erp, o.order_sn, o.add_time,o.pay_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.rebate_ispay, o.pay_name,o.wx_refund_status," .  //MOD by PRINCE
                "(" . order_amount_field('o.') . ") AS total_fee, " .
                "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,supplier_name,o.froms,is_pickup  ".
                /*增值税发票_添加_START_www.68ecshop.com*/
    ...
    foreach ($row AS $key => $value)
    {
        $row[$key]['user_name'] = $GLOBALS['db']->getOne("select user_name from " . $GLOBALS['ecs']->table('users') . " where user_id = '" . $row[$key]['user_id']. "'");
        $row[$key]['formated_order_amount'] = ($value['order_amount']);
        $row[$key]['formated_money_paid'] = ($value['money_paid']);
        $row[$key]['formated_total_fee'] = ($value['total_fee']);
        $row[$key]['short_order_time'] = local_date('m-d H:i', $value['add_time']);
        /*增值税发票_添加_START_www.68ecshop.com*/
        $row[$key]['formatted_add_time'] = local_date('Y-m-d H:i',$value['add_time']);
        $row[$key]['formatted_pay_time'] = local_date('Y-m-d H:i',$value['pay_time']);
        $row[$key]['formatted_inv_money'] = ($value['inv_money']);
        /*增值税发票_添加_END_www.68ecshop.com*/
        if ($value['order_status'] == OS_INVALID || $value['order_status'] == OS_CANCELED)
        {
            /* 如果该订单为无效或取消则显示删除链接 */
        ...
        $res = $GLOBALS['db']->query($sql);
        while ($row_goods = $GLOBALS['db']->fetchRow($res))
        {
            /* 虚拟商品支持 */
            if ($row_goods['is_real'] == 0)
            {
                /* 取得语言项 */
                $filename = ROOT_PATH . 'plugins/' . $row_goods['extension_code'] . '/languages/common_' . $_CFG['lang'] . '.php';
                if (file_exists($filename))
                {
                    include_once($filename);
                    if (!empty($_LANG[$row_goods['extension_code'].'_link']))
                    {
                        $row_goods['goods_name'] = $row_goods['goods_name'] . sprintf($_LANG[$row_goods['extension_code'].'_link'], $row_goods['goods_id'], $order['order_sn']);
                    }
                }
            }

            //手机专享价格修改 app  jx
            if($row_goods['exclusive'] == '-1')
            {
                $row_goods['formated_subtotal']       = ($row_goods['goods_price'] * $row_goods['goods_number']);
                $row_goods['formated_goods_price']    = ($row_goods['goods_price']);
                $row_goods['exclusive'] = "没有手机专享价格";
            }else
            {
                if($row_goods['exclusive'] > $row_goods['goods_price'])
                {
                    $row_goods['formated_subtotal']       = ($row_goods['goods_price'] * $row_goods['goods_number']);
                    $row_goods['formated_goods_price']    = ($row_goods['goods_price']);
                    $row_goods['exclusive'] = "没有手机专享价格";
                ...
                {
                    $row_goods['formated_subtotal']       = ($row_goods['exclusive'] * $row_goods['goods_number']);
                    $row_goods['formated_goods_price']    = ($row_goods['goods_price']);
                    $row_goods['exclusive'] = ($row_goods['exclusive']);
                }
            }

            //手机专享价格修改 app  jx

            $goods_attr[] = explode(' ', trim($row_goods['goods_attr'])); //将商品属性拆分为一个数组
            $row_goods['goods_attr']=str_replace(array("\n","\r"),array("",""),$row_goods['goods_attr']);
            if ($row_goods['extension_code'] == 'package_buy')
            {
                $row_goods['storage'] = '';
                $row_goods['brand_name'] = '';
                $row_goods['package_goods_list'] = get_package_goods($row_goods['goods_id'], $row_goods['package_attr_id']); //修改 by www.ecshop68.com
            }

            $sql_back = "SELECT bg.*, bo.back_type FROM " . $GLOBALS['ecs']->table('back_goods') . " AS bg " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('back_order') . " AS bo " .
                " ON bg.back_id = bo.back_id " .
                " WHERE bo.order_id = " . $value['order_id'] .
                " AND bg.goods_id = " . $row_goods['goods_id'] .
                " AND bg.product_id = " . $row_goods['product_id'] .
                " AND bg.status_back < 6";
            $back_info = $GLOBALS['db']->getRow($sql_back);
            ...
            $row_goods['shouhou'] = $shouhou;

            $goods_list[] = $row_goods;
        }
