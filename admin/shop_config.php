...
if ($_REQUEST['act'] == 'list_edit')
{
    ...
    if ($_CFG['shop_country'] > 0)
    {
     ...
     // 获得快递100的配置信息
    $kudidi_coustom = $db->getOne('select value from ecs_shop_config where code="kuaidi100"');
    $kudidi_key = $db->getOne('select value from ecs_shop_config where code="kuaidi100_api"');

    $smarty->assign('kuadi_coust', $kudidi_coustom);
    $smarty->assign('kuaidii_key', $kudidi_key);
...
elseif ($_REQUEST['act'] == 'post')
{
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                if ($code == 'shop_logo')
                {
                    ...
                    if(intval($file['size']) > 500 * 1024){
                        sys_msg(sprintf($_LANG['upload_limit'], $_LANG['cfg_range']['upload_size_limit']['500']));
                    }
                    $ext = array_pop(explode('.', $file['name']));
                    $new_name = 'wx_supplier0'.'.'.$ext;//对店铺wx重启名称
                    $to_path =  ROOT_PATH.DATA_DIR.'/supplier/wx/';
                    if(!file_exists($to_path)){
                        mkdir($to_path,0777);
                    }
                    $file_name = $to_path.$new_name;
                }
                ...
                {
                    if($code == 'wx'){
                        $sql = "UPDATE " . $ecs->table('shop_config') . " SET value = '".str_replace(ROOT_PATH,'',$file_name)."' WHERE code = '$code'";
    ...
    // 更换快递100 的配置 addby yaolianmin
    if($_POST['kuaidi_custom'] && $_POST['kuaidi_key']){

        $sql = 'update ecs_shop_config set value="'.$_POST['kuaidi_custom'].'" where code="kuaidi100"';
        $db->query($sql);
       
        $sql = 'update ecs_shop_config set value="'.$_POST['kuaidi_key'].'" where code="kuaidi100_api"';
        $db->query($sql);
