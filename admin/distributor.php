...
$image = new cls_image($_CFG_MOBILE['bgcolor']);
...
if ($_REQUEST['act'] == 'list')
{
	...
	//判断是否是分销商
	$distrib_rank = $_CFG_MOBILE['distrib_rank'];
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	//修改分销商更新会员购物车 edit by yhy2019/5/18
...
elseif ($_REQUEST['act'] == 'edit_username')
{
    ...
    {
        if ($_CFG_MOBILE['integrate_code'] != 'ecshop')
        {
            /* 更新商城会员表 */
...
elseif ($_REQUEST['act'] == 'aff_list')
{
    ...
    {
        $user_list['user_list'][$i]['reg_time'] = local_date($_CFG_MOBILE['date_format'], $user_list['user_list'][$i]['reg_time']);
    }
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['up_user_name'] = empty($_REQUEST['up_user_name']) ? '' : trim($_REQUEST['up_user_name']);
		...
		if(isset($filter['sign']))
		{
			...
			if(($filter['sign'] == 2||$filter['sign'] == 3)&&$filter['up_user_name'])
            {
                $sql = "SELECT user_id FROM ".$GLOBALS['ecs']->table('users')." where user_name LIKE '%" . mysql_like_quote($filter['up_user_name']) ."%'";;
                $user_lists = $GLOBALS['db']->getAll($sql);
                foreach ($user_lists as $key => $val){
                    $user_list_v[$key]=$val['user_id'];
                }
                $comma_separated = implode(",", $user_list_v);
                if(!empty($comma_separated))
                {
                    $ex_where .= " AND parent_id in ('$comma_separated') ";
                }
            }
			if($filter['sign'] == 4){
				$ex_where .= " AND parent_id = 0 AND is_fenxiao <> 1 ";
			}
		}
    ...
    for ($i=0; $i<$count; $i++)
    {
        $user_list[$i]['reg_time'] = local_date($GLOBALS['_CFG_MOBILE']['date_format'], $user_list[$i]['reg_time']);
