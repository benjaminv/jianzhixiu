...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ((array)$image_descs AS $key => $img_desc)
    {
        /* 是否成功上传 */
        $flag = false;
...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
      ...
      if ($filter['is_morning_market'])
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
        	$sql = "SELECT goods_id,g.sync_erp,g.sync_result, goods_name, goods_type,is_morning_market, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
        	...
        	$sql = "SELECT goods_id,sync_erp,sync_result, goods_name, goods_type, goods_sn,is_morning_market, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number, xg_goods_number
