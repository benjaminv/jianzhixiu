...
/**
 * 取得手机端广告位置数组（用于生成下拉列表）
 *
 * @return  array       分类数组 position_id => position_name
 */
function get_position_mobile_list()
{
    $position_list = array();
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
 ...
 * @return  array       分类数组 position_id => position_name
 */
function get_mobile_position_list()
{
    $position_list = array();
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['ecs']->table('ecsmart_ad_position');
 ...
 */
function create_html_editor($input_name, $input_value = '',$width=1000)
{
    global $smarty;
    $HTML='
...
$input_value.'</textarea>
    ...
    theme:"default", //皮肤
    lang:"zh-cn",    //语言
    initialFrameWidth:'.$width.',  //初始化编辑器宽度,默认650
