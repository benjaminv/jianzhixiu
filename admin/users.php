...
function action_insert ()
{
    ...
    $sql="select user_id from ". $ecs->table('users') ." where user_name = '$parent_username'";
	...
	if(isset($_POST['extend_field5']) && !empty($_POST['extend_field5'])){
		$other['mobile_phone'] = htmlspecialchars(trim($_POST['extend_field5']));
	}
    $other['user_status'] = isset($_POST['user_status']) ? htmlspecialchars(trim($_POST['user_status'])) : '';
	...
	$mobilesql = "";
	if(isset($mobile_phone) && !empty($mobile_phone)){
		$mobilesql = ',mobile_phone="'.$mobile_phone.'"';
	}
	$sql = "update " . $ecs->table('users') . " set  parent_id='$parent_id' ".$mobilesql." , `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' where user_name = '" . $username . "'";
	$db->query($sql);
	...
	if(ODOO_ERP){
		...
		//edit yhy 同步erp中的会员
		$res = $odooErpObj->syncUserByUserid($uid);
	}
...
function action_edit ()
{
	...
	/* 代码增加2014-12-23 by www.68ecshop.com _star */
	$sql = "SELECT u.user_status,u.issue, u.issue_address, u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
	...
	if($row)
	{
		...
		$user['issue'] = $row['issue'];
		$user['issue_address'] = $row['issue_address'];
		/* 代码增加2014-12-23 by www.68ecshop.com _end */
	}
	else
	{
...
function action_update ()
{
	...
	if($parent_id>0){
        ...
        $sql="select parent_id from ". $ecs->table('users') ." where  user_name = '$username'";
        $old_parent_id = $db->getOne($sql);
        //判断分校
        ...
        $sql="select user_id from ". $ecs->table('users') ." where user_rank = '$daili_rank_id' and user_name = '$username'";
        $One_user_id = $db->getOne($sql);
        $sql="select user_id from ". $ecs->table('users') ." where user_rank = '$fenxiao_rank_id' and user_id = '$parent_id'";
        $fenxiao_user_id = $db->getOne($sql);
        if(!empty($One_user_id)&&!empty($fenxiao_user_id)&&$old_parent_id!=$parent_id)
        {
            ...
            daili_jiaru($parent_id,$One_user_id);
            // rank_change($user_id,$old_rank_name['rank_name'],$now_rank_name['rank_name']);
        }

        $sql = "update " . $ecs->table('users') . " set `parent_id` = '$parent_id' where user_name = '" . $username . "'";
        $db->query($sql);
	}
	if($face_card != '')
	{
    ...
    //会员等级变更
    $old_rank_name = $db->getRow("SELECT uk.rank_name,uk.rank_id FROM " . $ecs->table('user_rank') . " as uk left join ".$ecs->table('users')." as u on uk.rank_id = u.user_rank WHERE user_id = '$user_id'");
    $old_rank_name['rank_name'] = ! empty($old_rank_name['rank_name']) ? trim($old_rank_name['rank_name']) : "普通会员";
    $old_rank_name['rank_id'] = ! empty($old_rank_name['rank_id']) ? ($old_rank_name['rank_id']) : "0";
    $now_rank_name=$db->getRow("SELECT rank_name,rank_id FROM " . $ecs->table('user_rank') . "  WHERE rank_id = '$rank'");
    $now_rank_name['rank_name'] = ! empty($now_rank_name['rank_name']) ? trim($now_rank_name['rank_name']) : "普通会员";
    $now_rank_name['rank_id'] = ! empty($now_rank_name['rank_id']) ? ($now_rank_name['rank_id']) : "0";
	/* 更新会员的其它信息 */
	$other = array();
	$other['credit_line'] = $credit_line;
	$other['user_rank'] = $rank;
	...
	**/
	...
	if($now_rank_name['rank_id']!=$old_rank_name['rank_id'])
	{
        /*  微信通知 */
        $_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'] ? $_SERVER['REQUEST_URI'] : "/mobile/";
        $autoUrl = str_replace($_SERVER['REQUEST_URI'],"",$GLOBALS['ecs']->url());
...
function action_edit_username ()
{
	...
	if($users->edit_user($id, $username))
	{
		...
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			//edit yhy 同步erp中的会员
			$res = $odooErpObj->syncUserByUserid($id);
		}
...
function action_remove ()
{
	...
	if($issupplier>0){
		...
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			//edit yhy 同步erp中的会员
			$res = $odooErpObj->syncUserUnlinkByUserid($_GET['id']);
		}
		/* 记录管理员操作 */
...
function user_list ()
{
	...
	if($result === false)
	{
        ...
        $filter['user_status'] = empty($_REQUEST['user_status']) ? '' : trim($_REQUEST['user_status']);
        if($filter['user_status'])
        {
            $ex_where .= " AND  u.user_status LIKE '%" . mysql_like_quote($filter['user_status']) . "%' ";
        }
