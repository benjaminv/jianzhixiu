...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['ads_add'], 'href' => 'mobile_ads.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('mobile_ads_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('mobile_ads_list.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link',   array('href' => 'mobile_ads.php?act=list', 'text' => $_LANG['ad_list']));
    $smarty->assign('position_list', get_position_mobile_list());
    ...
    assign_query_info();
    $smarty->display('mobile_ads_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($_POST['media_type'] == '0')
    {
        ...
        {
            $img_path = $image->upload_image($_FILES['ad_img'], '../mobile/data/afficheimg');
            $ad_code = basename($img_path);
    ...
    $link[0]['href'] = 'mobile_template.php?act=setup';

    $link[1]['text'] = $_LANG['back_ads_list'];
    $link[1]['href'] = 'mobile_ads.php?act=list';

    $link[2]['text'] = $_LANG['continue_add_ad'];
    $link[2]['href'] = 'mobile_ads.php?act=add';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($type == 0)
    {
        if ((isset($_FILES['ad_img']['error']) && $_FILES['ad_img']['error'] == 0) || (!isset($_FILES['ad_img']['error']) && isset($_FILES['ad_img']['tmp_name']) && $_FILES['ad_img']['tmp_name'] != 'none'))
        {
            ...
            $img_up_info = basename($image->upload_image($_FILES['ad_img'], '../mobile/data/afficheimg'));
   ...
   /* 提示信息 */
   $href[] = array('text' => $_LANG['back_ads_list'], 'href' => 'mobile_ads.php?act=list');
...
elseif ($_REQUEST['act'] == 'add_js')
{
    ...
    assign_query_info();
    $smarty->display('mobile_ads_js.htm');
}
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'mobile_ads.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
