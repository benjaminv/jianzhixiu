...
if($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_link' , array('href' => 'mobile_website.php?act=init' , 'text' => $_LANG['init']));
	$smarty->assign('ur_here' , $_LANG['ur_here']);
	$smarty->assign('website' , $web);
	assign_query_info();
	$smarty->display('mobile_website.htm');
}
elseif($_REQUEST['act'] == 'batch')
{
	...
	{
		$link[0] = array('href' => 'mobile_website.php?act=list' , 'text' => $_LANG['webstte_list']);
	...
	switch($type)
	{
		...
		case 'create':
			...
			$smarty->assign('action_link' ,  array('href' => 'mobile_website.php?act=list' , 'text' => $_LANG['webstte_list']));
			$smarty->display('mobile_website_view.htm');
			exit();
			break;
		case 'uninstall':
			break;
...
elseif($_REQUEST['act'] == 'install' || $_REQUEST['act'] == 'view')
{
	...
	$type = getChar('type');
	if(!$type) header('Location: mobile_website.php?act=list');
	$filepath = ROOT_PATH . 'mobile/includes/website/';
	...
	assign_query_info();
	$smarty->display('mobile_website_install.htm');
}
...
elseif($_REQUEST['act']  == 'query_install' || $_REQUEST['act'] == 'update_website')
{
	...
	$commnet .=	'?>';
	$filename = ROOT_PATH . 'mobile/includes/website/config/'.$type.'_config.php';
