...
switch ($act){
	...
	case "addmenu"://添加菜单
		...
		if($_POST){
			...
			$value = $_POST['value_wx'];
			$type = intval($_POST['type']);
			$order = intval($_POST['order']);
			$appid = getstr($_POST['appid']);
			$pagepath = getstr($_POST['pagepath']);
			if(!$id)
			{
				if($pid == 0)
				{
					$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('weixin_menu') . " WHERE pid = 0";
			...
			if($id){
				...
				$ret = $db->query ( "INSERT INTO " . $GLOBALS['ecs']->table('weixin_menu') . " (`pid`, `name`, `type`, `value`,`order`,`appid`,`pagepath`)
					VALUES('$pid', '$name', $type, '$value','$order','$appid','$pagepath');" );
			}
			update_menu();
			if ($ret) {
				sys_msg ( '添加成功', 0, $link );
		...
		if($_POST){
			...
			if($content){
				if(!$id){
					...
					if($count == 0)
					{
						require('../mobile/weixin/wechat.class.php');
	...
	case "oauth":
		...
		if($_GET['t'] == 'add' || $_GET['t'] == 'delete'){
			...
			if($_POST){
				$weburl = $_POST['weburl'];
				if($oid > 0){
					$ret = $db->query("UPDATE " . $GLOBALS['ecs']->table('weixin_oauth') . " SET weburl='$weburl',id=$id WHERE `oid`={$oid}" );
				}else{
					$ret = $db->query("INSERT INTO " . $GLOBALS['ecs']->table('weixin_oauth') . "(weburl,click,id) VALUE ('$weburl',0,$id)");
				}
...
function update_menu($id=1){
			...
			if($ret2){
				foreach($ret2 as $kk=>$vv){
					...
					if($vv['type'] == 1){
						...
						$button[$k]['sub_button'][$kk]['appid'] = $vv['appid'];
						$button[$k]['sub_button'][$kk]['pagepath'] = $vv['pagepath'];
					}else{
				...
				if($v['type'] == 1){
					...
					$button[$k]['appid'] = $v['appid'];
					$button[$k]['pagepath'] = $v['pagepath'];
	...
	if($res === false){
		sys_msg ( '更新菜单出错：'.$weixin->errMsg.";错误码：".$weixin->errCode, 1, $link );
	}else{
		return true;
	}
}
