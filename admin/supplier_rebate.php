...
elseif ($_REQUEST['act'] == 'export_goods')
{
	...
	$export = "<table border='1'><tr><td colspan='2'>账户变动时间</td><td colspan='2'>订单号</td><td colspan='2'>订单金额（元）</td><td colspan='2'>平台扣除佣金（元）</td><td colspan='2'>平台扣除分成佣金（元）</td><td colspan='2'>商家实际收入金额（元）</td><td colspan='2'>支付方式</td><td colspan='2'>备注</td></tr>";

	$result = supplier_rebate_info_list();
	foreach($result['result'] as $key=>$val)
	{
		$export .= "<tr><td colspan='2'>".$val['add_time']."</td><td colspan='2' style='vnd.ms-excel.numberformat:@'>".$val['order_sn']."</td><td colspan='2'>".$val['all_money']."</td><td colspan='2'>-".$val['rebate_money']."</td><td colspan='2'>".$val['split_money']."</td><td colspan='2'>".$val['result_money']."</td><td colspan='2'>".$val['pay_name']."</td><td colspan='2'>".$val['texts']."</td></tr>";
...
function supplier_rebate_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT sum(sr.all_money) as all_money, sum(sr.rebate_money) as rebate_money, sum(sr.result_money) as result_money,sum(sr.split_money) as split_money, sr.supplier_id, s.supplier_name, s.supplier_rebate ".
...
{
	global $db,$ecs;
	$sql = "select sum(all_money) as all_money,sum(rebate_money) as rebate_money,sum(result_money) as result_money ,sum(split_money) as split_money from ".$ecs->table('supplier_rebate_log');
...
//当前入驻商详细信息中的支付方式
function get_rebate_pay($suppid=0)
{
	global $db,$ecs;
	$suppid = intval($_REQUEST['suppid']);
