...
require_once('includes/lib_template_mobile.php');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 获得当前的模版的信息 */
    $curr_template = 'dm299_com';//$_CFG['template']
    $curr_style = $_CFG['stylename'];

    /* 获得可用的模版 */
    $available_templates = array();
    $template_dir        = @opendir(ROOT_PATH . 'mobile/themesmobile/');
    while ($file = readdir($template_dir))
    {
        if ($file != '.' && $file != '..' && is_dir(ROOT_PATH. 'mobile/themesmobile/' . $file) && $file != '.svn' && $file != 'index.htm')
        {
    ...
    $tmp_bak_dir = @opendir(ROOT_PATH . 'mobile/temp/backup/library/');
    while ($file = readdir($tmp_bak_dir))
    {
        if ($file != '.' && $file != '..' && $file != '.svn' && $file != 'index.htm' && is_file(ROOT_PATH .'mobile/temp/backup/library/' . $file) == true)
        {
            $code = substr($file, 0, strpos($file, '-'));
            if (!in_array($code, $available_code))
            {
                @unlink(ROOT_PATH . 'mobile/temp/backup/library/' . $file);
            }
        }
    }
    ...
    $smarty->display('mobile_templates_list.htm');
}
...
if ($_REQUEST['act'] == 'setup')
{
    ...
    $template_theme = 'dm299_com';//$_CFG['template']
	...
	if( !empty($page_libs[$curr_template]) ) {
    ...
    $smarty->assign('arr_ad_positions',   get_mobile_position_list());//var_dump(get_position_list());exit;
    $smarty->display('mobile_template_setup.htm');
}
...
if ($_REQUEST['act'] == 'setting')
{
    ...
    /* 修改模板文件 */
    $template_file    = '../mobile/themesmobile/' . $curr_template . '/' . $_POST['template_file'] . '.dwt';
    ...
    foreach ($org_regions AS $region)
    {
        ...
        foreach ($post_regions AS $lib)
        {
            ...
            {
                if (!file_exists('../mobile/themesmobile/' . $curr_template . $lib['library']))
                {
                    continue;
                }
                $lib_content     = file_get_contents('../mobile/themesmobile/' . $curr_template . $lib['library']);
    ...
    if (file_put_contents($template_file, $template_content))
    {
        ...
        clear_cache_files();
        $lnk[] = array('text' => $_LANG['go_back'], 'href'=>'mobile_template.php?act=setup&template_file=' .$_POST['template_file']);
        sys_msg($_LANG['setup_success'], 0, $lnk);
    }
    else
    {
        sys_msg(sprintf($_LANG['modify_dwt_failed'], 'mobile/themesmobile/' . $curr_template. '/' . $_POST['template_file'] . '.dwt'), 1, null, false);
    }
}
...
if ($_REQUEST['act'] == 'library')
{
    ...
    {
        /* 取得语言项 */
        if (file_exists(ROOT_PATH . 'mobile/plugins/'.$row['code'].'/languages/common_'.$_CFG['lang'].'.php'))
        {
            include_once(ROOT_PATH . 'mobile/plugins/'.$row['code'].'/languages/common_'.$_CFG['lang'].'.php');
        }
    }
    $curr_template = $_CFG['template'];
    $arr_library   = array();
    $library_path  = '../mobile/themesmobile/' . $curr_template . '/library';
    ...
    $smarty->display('mobile_template_library.htm');
}
...
if ($_REQUEST['act'] == 'backup')
{
    ...
    $filename = '../mobile/temp/backup/' . $tpl . '_' . date('Ymd') . '.zip';

    $zip = new PHPZip;
    $done = $zip->zip('../mobile/themesmobile/' . $tpl . '/', $filename);
...
if ($_REQUEST['act'] == 'update_library')
{
    ...
    $lib_file = '../mobile/themesmobile/' . $_CFG['template'] . '/library/' . $_POST['lib'] . '.lbi';
    ...
    {
        @file_put_contents('../mobile/temp/backup/library/' . $_CFG['template'] . '-' . $_POST['lib'] . '.lbi', $org_html);

        make_json_result('', $_LANG['update_lib_success']);
    }
    else
    {
        make_json_error(sprintf($_LANG['update_lib_failed'], 'mobile/themesmobile/' . $_CFG['template'] . '/library'));
    }
}
...
if ($_REQUEST['act'] == 'restore_library')
{
    ...
    $lib_file   = '../mobile/themesmobile/' . $_CFG['template'] . '/library/' . $lib_name . '.lbi';
    $lib_file   = str_replace("0xa", '', $lib_file); // 过滤 0xa 非法字符
    $lib_backup = '../mobile/temp/backup/library/' . $_CFG['template'] . '-' . $lib_name . '.lbi';
...
if ($_REQUEST['act'] == 'backup_setting')
{
    ...
    $smarty->display('mobile_templates_backup.htm');
}
...
if ($_REQUEST['act'] == 'restore_backup')
{
    ...
    if ($remarks)
    {
        ...
        if ($arr)
        {
            ...
            foreach ($arr as $val)
            {
                $lib_content     = file_get_contents(ROOT_PATH . 'mobile/themesmobile/' . $_CFG['template'] . $val['library']);
                //去除lib头部
            ...
            foreach ($data as $file => $regions)
            {
                ...
                $temple_file = ROOT_PATH . 'mobile/themesmobile/' . $_CFG['template'] . '/' . $file . '.dwt';
...
function load_library($curr_template, $lib_name)
{
    ...
    $lib_file    = '../mobile/themesmobile/' . $curr_template . '/library/' . $lib_name . '.lbi';
...
function read_tpl_style($tpl_name, $flag=1)
{
    ...
    $start = 0;
    $available_templates = array();
    $dir = ROOT_PATH . 'mobile/themesmobile/' . $tpl_name . '/';
