...
//send_order_bonus(724,0);
//echo date('Y-m-d H:i:s',local_gettime());exit;
/* 载入语言文件 */
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/user.php');
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/shopping_flow.php');

//$_LANG['use_points_payment'] = '积分抵扣';
//购物车打开修改商品属性页面
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	//结算的时候,更新购物车 
	update_user_cart($_SESSION['user_id']);
    ...
    if ($db->getOne($sql) == 0)
    {
        ...
        if($flow_type != CART_EXCHANGE_GOODS) {
            ...
            $sql="select g.points_payment_amount, c.goods_number,g.goods_id, g.goods_name,g.is_buy, g.buymax, g.buymax_start_date, g.buymax_end_date from ".$ecs->table('cart'). " AS c left join ".$ecs->table('goods'). " AS g on c.goods_id=g.goods_id where 1 ".$sql_plus;

            $goods_list = $db->getAll($sql);

            foreach($goods_list as $k => $v) {
                if ($v['points_payment_amount'] > 0) {
                    $cart_exist_point_goods = true;
                }
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    ...
    if ($flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_GROUP_BUY_GOODS)
    {
        foreach($cart_goods as $key => $value){
            $xz_flag=xz_number($value['goods_id'],$value['goods_number'],$value['goods_attr_id']);
            if($xz_flag==false){
                clear_cart($flow_type);
                clear_all_files();
                show_message($value['goods_name']."超过本月限购数量", "返回重新购物", 'index.php',  'info',  false);
            }
        }
    }
    ...
    if(isset($payment_list))
    {
        ...
        {
            if($payment['pay_code']=='alipay' && strpos($_SERVER['HTTP_USER_AGENT'], 'miniProgram')>0 )
            {   
    ...
    /**
     * @author 一脸大胡子
     * 1. 检查购物车中商品是否有可用积分抵扣的商品
     * 2. 如果有，获取客户可用积分，获取系统设置的1元=多少积分参数，计算此单可用积分，计算抵扣金额
     * 3. 如果没有，此单不可用积分抵扣
     */
    $pointsComputeResult = array();
    if ($cart_exist_point_goods) {
        require(ROOT_PATH . '../includes/lib_points.php');
        $pointsComputeResult = computeCartGoodsPointsAvailable($_SESSION['user_id'], $cart_goods);
    }

    $smarty->assign('customer_available_points', $pointsComputeResult ? $pointsComputeResult['customer_available_points'] : 0);
    $smarty->assign('order_available_point', $pointsComputeResult ? $pointsComputeResult['order_available_point'] : 0);
    $smarty->assign('system_point_exchange', $pointsComputeResult ? $pointsComputeResult['system_point_exchange'] : 0);
    $smarty->assign('deductible_amount', $pointsComputeResult ? $pointsComputeResult['deductible_amount'] : 0);
    ...
    if ((!isset($_CFG['use_bonus']) || $_CFG['use_bonus'] == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)){
		...
		//echo "<pre>";print_r($user_bonus);exit;
...
elseif ($_REQUEST['step'] == 'compute_points') {
    include_once('includes/cls_json.php');
    ...
    else
    {
        ...
        {
            if ($points > 0) {
                $order['use_point'] = true;
                /*获取积分兑换方案*/
                require(ROOT_PATH . '../includes/lib_points.php');
                $pointsComputeResult = computeCartGoodsPointsAvailable($_SESSION['user_id'], $cart_goods);
            } else {
                $order['use_point'] = false;
                $pointsComputeResult['deductible_amount'] = 0;
            }

            $order['deductible_amount'] = $pointsComputeResult['deductible_amount'];
            ...
            $result['deductible_amount'] = $pointsComputeResult['deductible_amount'];

            $smarty->assign('pointsComputeResult', $pointsComputeResult);

            if ($pointsComputeResult['deductible_amount'] >= $total['amount']) {
                $result['hide_pay_div'] = true; //隐藏支付方式
            } else {
                $result['hide_pay_div'] = false;
            }
...
{	
    ...
    $point_case = array();
    /*采用积分抵扣商品金额*/
    if (intval($_POST['ECS_USE_POINTS']) == 1) {
        require(ROOT_PATH . '../includes/lib_points.php');
        $point_case = computeCartGoodsPointsAvailable($_SESSION['user_id'], $cart_goods);
        $cart_goods = $point_case['goods'];
    }

	//判断是否有秒杀商品，如果有不能与其他商品一块购买
    $k =0;
    $j = 0;

    foreach($cart_goods as $key=>$value){
		$sql = "select s.* from ". $GLOBALS['ecs']->table('goods') . " g"." LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " s ON g.goods_id = s.goods_id"." where g.goods_id = $value[goods_id] and s.is_seckill=1";
        $seckill = $db->getRow($sql);
        $time = gmtime();
        if ($seckill['is_seckill'] == 1 && $time >= $seckill['seckill_start_date'] && $time <= $seckill['seckill_end_date']) {
            $k++;
            if($k == 1){
                $j = $seckill['seckill_id'];
            }else{
                if($seckill['seckill_id'] != $j){
                    sys_msg("请单独购买一个系列的秒杀商品");
                    break;
                }
            }
        }
    }
    if($k!=0 and $k != count($cart_goods)){
        show_message("秒杀商品请单独下单");
    }
    
    $goods_cheng = $cart_goods;
	...
	$order_use_point = intval($_POST['ECS_USE_POINTS']);
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
            ...
            'supplier_id'	  => $ckey,
            'seckill_id'      =>$j,
            'order_available_point' => $point_case['order_available_point'],
            'system_point_exchange' => $point_case['system_point_exchange'],
            'deductible_amount' => $point_case['deductible_amount'],
	        );
		...
		//秒杀商品  修改  by  xiaojun 
        foreach ($cart_goods as $a=>$b){
			//判断有没有秒杀商品 --锋
            $time = gmtime();
			$sql = "select s.* from ". $GLOBALS['ecs']->table('goods') . " g"." LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " s ON g.goods_id = s.goods_id"." where g.goods_id = $b[goods_id] and s.is_seckill=1 and s.seckill_start_date <= $time and  s.seckill_end_date>$time";				
            $seckill = $db->getRow($sql);
            if ($seckill){
                $order['extension_code'] = 150;
                break;
            }
        }		
    ...
    //判断是否有秒杀商品，如果有则判断数量是否足够 --锋
    foreach($goods_cheng as $key=>$value){
       // $sql = "select * from ".$ecs->table("goods")."where goods_id = $value[goods_id]";
		
        $sql = "select g.goods_name,g.goods_number,s.* from ". $GLOBALS['ecs']->table('goods') . " g"." LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " s ON g.goods_id = s.goods_id"." where g.goods_id = $value[goods_id] and s.is_seckill=1";		
        $scekill = $db->getRow($sql);
        $time = gmtime();
		
        if ($scekill['is_seckill'] == 1 && $time >= $scekill['seckill_start_date'] && $time <= $scekill['seckill_end_date'])
        {
           //判断秒杀数量是否足够
			//$num_skill= $value['goods_number']+$scekill['seckill_num'];
            if($value['goods_number']>$scekill['seckill_num']){
				/* 清空购物车 */
				clear_cart($flow_type,$id_ext);
				/* 清除缓存，否则买了商品，但是前台页面读取缓存，商品数量不减少 */
				clear_all_files();
                show_message("秒杀商品已售完",'返回首页','index.php');
				//商品被秒杀完了
            }else{
				
                //减少秒杀数量
                $sql = "update ".$ecs->table("seckill_goods")."set seckill_num = seckill_num-$value[goods_number] where goods_id = $value[goods_id] and is_seckill=1";
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    do
	    {
	        $order['order_sn'] = get_order_sn().$oi; //获取新订单号
	    ...
	    /* 代码增加_start  By www.68ecshop.com */
	    $fields = "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, ".
            "goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id, use_points_by_one, deduction_amount_by_one, system_point_exchange";

	    $sql_insert_order_goods = "INSERT INTO " . $ecs->table('order_goods') . " ( " . $fields . ") ";

	    $sql_query_cart_goods = " SELECT '$new_order_id' as 'order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id".
	            " FROM " .$ecs->table('cart') .
	            " WHERE $sql_where AND rec_type = '$flow_type' $id_ext_new ";

	    $temp_cart_goods = $GLOBALS['db']->getRow($sql_query_cart_goods);

        // 找出积分兑换方案中的商品积分抵扣金额
        foreach ($cart_goods as $v) {
            if ($v['goods_id'] == $temp_cart_goods['goods_id']) {
                $use_points_by_one = $v['use_points_by_one'] > 0 ? $v['use_points_by_one'] : 0;
                $deduction_amount_by_one = $v['deduction_amount_by_one'] > 0 ? $v['deduction_amount_by_one'] : 0;
                $system_point_exchange = $v['system_point_exchange'] > 0 ? $v['system_point_exchange'] : 0;

                $sql_query_cart_goods = " SELECT '$new_order_id' as 'order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id, {$use_points_by_one} as use_points_by_one, {$deduction_amount_by_one} as deduction_amount_by_one , {$system_point_exchange} as system_point_exchange".
                    " FROM " .$ecs->table('cart') .
                    " WHERE $sql_where AND rec_type = '$flow_type' $id_ext_new ";
                break;
            }
	    ...
	    /* 代码增加_end  By www.68ecshop.com */
	    $db->query($sql_insert_order_goods . $sql_query_cart_goods);

	    // 追加积分抵扣 $new_order_id
        foreach ($cart_goods as $v) {
            $p_goods_id = $v['goods_id'];
            $p_use_points_by_one = $v['use_points_by_one'];
            $p_deduction_amount_by_one = $v['deduction_amount_by_one'];
            $p_system_point_exchange = $v['system_point_exchange'];
            $sql = "UPDATE " . $ecs->table('order_goods') . " SET use_points_by_one = '$p_use_points_by_one', deduction_amount_by_one = '$p_deduction_amount_by_one', system_point_exchange = '$p_system_point_exchange' WHERE order_id = '$new_order_id' AND goods_id = '$p_goods_id'";
            $db->query($sql);
        }
        ...
        if (intval($order['user_id']) > 0 && $order['deductible_amount'] > 0)
        {
            log_account_change($order['user_id'], 0, 0, 0, $order['order_available_point'] * (-1), sprintf($_LANG['pay_order'], $order['order_sn']));
        }
		...
		$split_order['suborder_list'][$ok]['deductible_amount'] = $order['deductible_amount'];
		$split_order['suborder_list'][$ok]['deductible_amount_formated'] = price_format($order['deductible_amount'], false);
	            ...
	            {         
	                ...
	                if ($db->getOne($sql) <= 0)
	                {
	                    ...
	                    if ($order['user_id'] > 0)
	                    {
	                        ...
	                        /* 发放红包  更改为订单支付成功就发放红包，不区分是否为虚拟商品*/
	...
	if ($order['order_amount'] <= 0){
		//edit by yhy 2019/5/16 余额全额支付后发放订单红包
		//订单支付成功后发放红包 
		foreach($split_order['suborder_list'] as $k=>$v){
			send_order_bonus($v['order_id'],$k);
		}
		//yhy edit 2018/12/14 星期五 余额全额支付后同步订单到erp后台
		//exit;
		$ordersns = array_column($split_order['suborder_list'],'order_sn');
		if(ODOO_ERP){
   ...
   if($send){
	...
	if ($order['order_amount'] > 0)
    {
       ...
       if ($payment['pay_code']=='weixin'||$payment['pay_code']=='alipay'){
...
{
	//echo "<pre>";
    //echo $GLOBALS['_CFG']['template'];die;
	$smarty->assign('template_dir', $GLOBALS['_CFG']['template']);
...
function track_order($ordersns){
	if(TRACK){
		...
		foreach($orderInfos as $k=>$v){
			...
			$orderGoods = $GLOBALS['db']->getAll($orderGoodsSql);
...
//根据会员id更新会员购物车 yhy 2019/5/18
function update_user_cart($user_id){
	...
	if(isset($data) && !empty($data)){
	 ...
	 /* 取得该会员有可能改变价格的商品：除配件和赠品之外的商品 */
    $sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id,g.market_price, g.promote_price, g.promote_start_date, c.goods_number,'.
                "g.promote_end_date, IFNULL(mp.user_price, g.shop_price * '$rank[discount]') AS member_price ".
            'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c '.
            'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = c.goods_id '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '" . $rank['rank_id'] . "' ".
            "WHERE  c.user_id='".$user_id."' AND c.parent_id = 0  AND c.is_gift = 0 AND c.goods_id > 0 " .
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['ecs']->table('cart'). " SET market_price = '".$row['market_price']."', goods_price = '$goods_price' ".
                     "WHERE goods_id = '" . $row['goods_id'] . "' AND rec_id = '" . $row['rec_id'] . "'";
...
function get_rank($user_rank, $rank_points)
{
    ...
    if (!empty($row))
    {
        ...
        {
            return $db->getRow(
                'SELECT discount, rank_id FROM ' . $ecs->table('user_rank') . " WHERE min_points >= '$rank_points' AND special_rank=0 ORDER BY min_points ASC LIMIT 1"
            );
        }
    }
    else
    {
        return $db->getRow(
            'SELECT discount, rank_id FROM ' . $ecs->table('user_rank') . " WHERE '$rank_points' >= min_points AND special_rank=0 ORDER BY min_points DESC LIMIT 1"
        );
    }
}
