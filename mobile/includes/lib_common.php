...
function load_config()
{
	...
	$arr['one_step_buy']         = 0;
		...
		$_CFG['service_phone']       = intval($arr['service_phone']);
 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER,$ordersn = "")
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
	...
	//积分变动更新到单点登录系统
	if($pay_points !== 0 && SSO_USERCENTER){
		...
		$updateData=array(
			"user_id"=>	$portal_id,
			"credit_val"=>abs($pay_points),
			"ordersn"=>$ordersn,
			"datetime"=>date("Y-m-d H:i:s",$account_log['change_time']),
			"credit_desc"=>($pay_points>0)?"商城获取":"商城消耗",
			"credit_type"=>($pay_points>0)?'1':'0',
			"origin_use"=>$change_desc?$change_desc:"商城"
		);
		
		$a= $GLOBALS['ssoObj']->creditUpdate($updateData);
	}
}
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    if(!empty($exclusive) && $exclusive >0){
	    ...
	    //取得商品秒杀价格列表
    /* 取得商品信息 */
    $sql = "SELECT s.seckill_price, s.seckill_start_date, s.seckill_end_date, ".
        "IFNULL(mp.user_price, g.shop_price * '" . $_SESSION['discount'] . "') AS shop_price, g.exclusive ".
        " FROM " .$GLOBALS['ecs']->table('goods'). " AS g ".
        " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '" . $_SESSION['user_rank']. "' ".
		" LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " AS s ".
		"ON g.goods_id = s.goods_id and s.is_seckill>0 ".
        " WHERE g.goods_id = '" . $goods_id . "'" .
        " AND g.is_delete = 0";
    $seckill = $GLOBALS['db']->getRow($sql);
    
    /* 计算秒杀价格 */
    if ($seckill['seckill_price'] > 0)
    {
        $seckill_price = bargain_price($seckill['seckill_price'], $seckill['seckill_start_date'], $seckill['seckill_end_date']);
    ...
    //如果秒杀价格不为0
    if(!empty($seckill_price)){

        $final_price = $seckill_price;
    }
