...
function insert_query_info()
{
    ...
    //return true;//sprintf($GLOBALS['_LANG']['query_info'], $GLOBALS['db']->queryCount, $query_time, $online_count) . $gzip_enabled . $memory_usage . $cron_method;
}
...
function insert_get_shop_shipping($arr){
    ...
    $GLOBALS['smarty']->assign('special_rank',   $special_rank);
...
function insert_kefu(){
    $sql=" select value from " .$GLOBALS['ecs']->table('shop_config') ." where code = 'ww'";
    $baidu_sq = $GLOBALS['db']->GetOne($sql);
    $sql=" select value from " .$GLOBALS['ecs']->table('shop_config') ." where code = 'wx'";
    $wx_kf = $GLOBALS['db']->GetOne($sql);
    if($wx_kf)
    {
        return "/".$wx_kf;
    }
