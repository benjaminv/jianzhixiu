...
{
	$sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table('user_qrcode') . " where user_id='$user_id'";
...
{
	$sql = "SELECT * from " . $GLOBALS['ecs']->table('user_qrcode') . " WHERE user_id = '$user_id'";
	return $GLOBALS['db']->getRow($sql); 
}
...
//获取分销商下级会员信息,$level代表哪一级，1代表是一级会员
function get_distrib_user_daili_info($user_id,$level)
{
    ...
    for ($i = 1; $i<=$level; $i++)
    {
        ...
        if ($up_uid)
        {
            $sql = 'SELECT u.user_id '.
                ...
                " WHERE u.parent_id IN($up_uid) and u.user_rank = '$fenxiao_rank_id'";
            //$sql = "SELECT user_id FROM " . $GLOBALS['ecs']->table('users') . " WHERE parent_id IN($up_uid)";
            $query = $GLOBALS['db']->query($sql);
            $up_uid = '';
            while ($rt = $GLOBALS['db']->fetch_array($query))
...
function get_count_distrib_order_by_user_id($user_id,$is_separate)
{
	...
	if(!empty($oids))
	{
		$sql = "SELECT COUNT(*) FROM 
				...
				$GLOBALS['ecs']->table('order_goods') . " as og where o.order_id = og.order_id AND o.order_id in($oids) AND o.pay_status = 2 AND is_separate = '$is_separate'  AND ((o.bonus=0 AND o.bonus_id=0) OR (o.bonus>0 AND o.bonus_id>0 AND t.is_distribute=1))  group by o.order_id ) as oog $where";
		return $GLOBALS['db']->getOne($sql);
	}
	return 0;
}
	...
	if(!empty($uids))
	{
		...
		if(!empty($oids))
		{
			$sql = "SELECT  order_id,goods_id,goods_name,user_id," . 
				...
				" AND o.pay_status = 2 AND is_separate = '$is_separate'  AND ((o.bonus=0 AND o.bonus_id=0) OR (o.bonus>0 AND o.bonus_id>0 AND t.is_distribute=1)) " .
			...
			while ($row = $GLOBALS['db']->fetchRow($res)) {
                ...
                if ($is_separate == 0)
                {
                    $arr[$row['order_id']]['total_money'] = affiliate_order_money_shouyi($row['order_id']);
                }
                else
                {
...
function affiliate_order_money_shouyi($oid)
{
    $sql = "select * from " .
        $GLOBALS['ecs']->table('order_info') .
        " where order_id = '$oid' ";
