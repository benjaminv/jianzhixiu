...
function get_user_orders_ajax($user_id, $limit,$where='')
{
	...
	$row['order_status1'] = $row['order_status'];
		...
		$arr[$row['order_id']] = array(
						...
						'shaidan_s'      => $shaidan_s,
			             'tk'            => $row['tk'],
						'total_fee'      => price_format($row['total_fee'] - $row['deductible_amount'], false),
...
function cancel_order($order_id, $user_id = 0)
{
    ...
    if ($GLOBALS['db']->query($sql))
    {
        ...
        foreach ($seckill_goods as $k=>$v){
			...
			$sql = "select s.* from ". $GLOBALS['ecs']->table('goods') . " g"." LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " s ON g.goods_id = s.goods_id"." where g.goods_id = $v[goods_id] and s.is_seckill=1";			
            ...
            if($time >= $seckill_data['seckill_start_date'] && $time <= $seckill_data['seckill_end_date'] && $seckill_data['is_seckill'] == 1){
                ...
                $sql = "update ".$GLOBALS['ecs']->table("seckill_goods")."set seckill_num = seckill_num+$v[goods_number] where goods_id = $v[goods_id]";
...
{
    $sql = "SELECT u.bonus_sn, u.order_id, u.supplier_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date,u.bonus_money,b.type_id,b.goods_use ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		if($row['bonus_money']){
			$row['type_money'] = $row['bonus_money'];
		}
        $arr[] = $row;
    }
    return $arr;
