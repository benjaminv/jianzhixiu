...
class weixin {
	...
	function get_code($order, $payment) {
		...
		$selfUrl = 'https://' . $_SERVER ['HTTP_HOST'] . $_SERVER ['PHP_SELF'] . '?' . $_SERVER ['QUERY_STRING'];

		if(! isset($_GET['payopenid']) && !isset($_SESSION['payopenid'])){
			$url = 'https://hljm.linkall.cn.com/app/index.php?i=1&c=auth&a=wxpay&callback='.$selfUrl;
			Header ( "Location: $url" );exit;
		}else{
			$_SESSION['payopenid'] = $_GET ['payopenid'];
		}
	...
	function natpayHtml($order) {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
			...
			// 设置统一支付接口参数
			$return_url = 'https://' . $_SERVER ['HTTP_HOST'].'/respond.php';
			$unifiedOrder->setParameter ( "body", $order ['order_sn'] );
			$unifiedOrder->setParameter ( "out_trade_no", $order ['order_id'] ); // 商户订单号
			$unifiedOrder->setParameter ( "total_fee", $order ['order_amount'] * 100 ); // 总金额
			...
			$product_url = $unifiedOrderResult["code_url"];
			 $button = "<img src='https://qr.liantu.com/api.php?text=" . $product_url . "' alt='扫描进行支付'>";
