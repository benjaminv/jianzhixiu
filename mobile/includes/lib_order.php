 ...
 */
function available_shipping_list($region_id_list,$suppid=0,$is_default_show=2,$shipping_code=null)
{
	...
	//物流公司bug同一个物流不同地区会在页面上显示多个物流 edit by yhy 2019/5/14
	$where = ' a.`enabled`=1 AND a.`supplier_id`='.$suppid.' AND c.`region_id` '.db_create_in($region_id_list);
	if($is_default_show == 0 || $is_default_show == 1){
		$where .= ' AND a.`is_default_show`=' . $is_default_show;
	}

	if(!empty($shipping_code)){
		if($shipping_code == 'pups'){
			$where .= ' AND a.`shipping_code` = "' . $shipping_code . '" ';
		}else if($shipping_code == 'tc_express'){
			$where .= ' AND a.`shipping_code` = "' . $shipping_code . '" ';
		}else{
			$where .= ' AND a.`shipping_code` != "pups" AND a.`shipping_code` != "tc_express"  ';
		}
	}

	$sql = 'SELECT a.`shipping_id`, a.`shipping_code`,a.`shipping_name`,a.`shipping_desc`, a.`insure`, a.`support_cod`,b.`configure`,a.`support_pickup` 
			FROM '.$GLOBALS['ecs']->table('shipping'). ' a 
			LEFT JOIN '.$GLOBALS['ecs']->table('shipping_area').' b ON a.shipping_id=b.shipping_id 
			LEFT JOIN '.$GLOBALS['ecs']->table('area_region').' c ON b.shipping_area_id=c.shipping_area_id 
			WHERE ' . $where . ' ORDER BY a.`shipping_order` desc';;
	;
    //end
    return $GLOBALS['db']->getAll($sql);
}
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            "goods_price * goods_number AS subtotal, extension_code, package_attr_id, use_points_by_one, system_point_exchange, deduction_amount_by_one " .
...
function order_fee($order, $goods, $consignee)
{
    ...
    {
        $bonus          = bonus_info($order['bonus_id'],"",false,$goods);
    ...
    {
        $bonus          = bonus_info(0,$order['bonus_sn'],false,$goods);
        $total['bonus'] += $bonus['type_money'];
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		foreach ($goods AS $val)
		{
		   ...
		   //会员等级
            $sql = 'SELECT ur.special_rank FROM ' . $GLOBALS['ecs']->table('users') . " AS u left join ". $GLOBALS['ecs']->table('user_rank') ." AS ur on u.user_rank=ur.rank_id WHERE u.user_id = '". $_SESSION['user_id'] ."'";  //jx
            $special_rank = $GLOBALS['db']->getOne($sql);
			
            if($special_rank==1)
            {
				
                $total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee'] =shipping_fee($shipping_info['shipping_code'],$shipping_info['configure'], $weight_price2['weight'], $total['goods_price_supplier'][$row_supp['supplier_id']], $weight_price2['number']);
            }
            else
            {
    ...
    $total['use_point'] = $order['use_point'];

    /*积分*/
    if ( $order['use_point'] || $order['deductible_amount'] > 0) {
        $total['amount'] -= $order['deductible_amount'];
        $total['deductible_amount'] = $order['deductible_amount'];
    } else {
        $total['amount'] += $order['deductible_amount'];
        $total['deductible_amount'] = 0;
    }

    $total['deductible_amount_format'] = price_format($total['deductible_amount'], false);
    $order['deductible_amount_format'] = price_format($order['deductible_amount'], false);
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.goods_attr_id,c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.xg_goods_number, g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
    ...
    {
        $product_info = array('product_number' => '','xg_goods_number' => '', 'product_id' => 0);
    }
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage)
            {
				...
				$dat="user_id:".$_SESSION['user_id'].",goods_id:".$goods_id .",real_user_rank:".$real_user_rank .",user_rank:".$_SESSION['user_rank'].",goods_price:".$goods_price.",goods_attr:".get_goods_attr_info($spec).",type:UPDATE".",time:".date("Y-m-d h:i:sa");
                write_log($dat);
            }
            else
            {
        ...
        else //购物车没有此物品，则插入
        {
			...
			//添加日志
            $dat="user_id:".$parent['user_id'].",goods_id:".$parent['goods_id'].",real_user_rank:".$real_user_rank.",user_rank:".$_SESSION['user_rank'].",goods_price:".$parent['goods_price'].",goods_attr:".$parent['goods_attr'].",type:INSERT".",time:".date("Y-m-d h:i:sa");
            write_log($dat);
        }
    }
...
//        }
	...
	if(SSO_USERCENTER){
		...
		$cerditInfo = $GLOBALS['ssoObj']->creditGetByUserid($user_id);
		if(isset($cerditInfo['code']) && $cerditInfo['code'] == 200){
			$user_points = $cerditInfo['credit'];
		}else{
			$user_points = 0;
		}
		$user['pay_points'] = $user_points;
	}
    return $user;
}
 ...
 * @return  array   红包数组
 */
function user_bonus($user_id, $goods_amount = 0,$cart_goods=array())
{
		$supplier_carts_goods=array();
		foreach($cart_goods as $k=>$v){
			$supplier_carts_goods[$v['supplier_id']][$v['goods_id']] = $v['subtotal'];
		}
    	
		foreach($goods_amount as $key=>$val)
		{
			$list = user_bonus_filter($user_id,$val,$key,$supplier_carts_goods[$key]);
			$res[$key] = $list;
		}
    	return $res;
}
...
**amount 购物车店铺订单金额 
**supplier_id  店铺id
**carts_goods  购物车店铺商品列表
*/
function user_bonus_filter($user_id,$amount,$supplier_id=0,$carts_goods){
	$day    = getdate();
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	//echo "<pre>";print_r(local_date('Y-m-d H:i:s','1558540800'));
	//echo "<pre>";print_r(local_date('Y-m-d H:i:s','1558627200'));
	//echo "<pre>";print_r(local_date('Y-m-d H:i:s',$today));
	
	$sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id, t.supplier_id,b.bonus_money,t.goods_use,t.use_goods_config,t.min_goods_amount " .
				...
				"AND t.min_goods_amount <= '$amount' " .
				"AND b.user_id<>0 " .
				"AND b.user_id = '$user_id' " .
				"AND b.order_id = 0 ".
				"AND t.supplier_id = $supplier_id";
	$list = $GLOBALS['db']->getAll($sql);
	$carts_goods_keys = array_keys($carts_goods);
	foreach($list as $k=>$v){
		if($v['bonus_money']){
			$list[$k]['type_money'] = $v['bonus_money'];
		}
		//指定商品使用的红包
		if($v['goods_use']){
			$use_goods_config = explode(',',$v['use_goods_config']);
			$public_keys = array_intersect($carts_goods_keys,$use_goods_config);
			if(!isset($public_keys) || empty($public_keys)){
				unset($list[$k]);
			}else{
				$goods_money = 0;
				foreach($public_keys as $value){
					$goods_money += $carts_goods[$value];
				}
				if($goods_money<$v['min_goods_amount']){
					unset($list[$k]);
				}
			}
		}
 ...
 * @param   bool    $flag		默认为正常情况下的红包，false表示下单时抵扣的金额(按照购物车的可抵扣商品价格返回红包可抵扣的金额)
 * @param   array   红包信息  
 */
function bonus_info($bonus_id, $bonus_sn = '',$flag = true,$goods = array())
{
	...
	if($res['type_money']){
		$res['type_money'] = $res['bonus_money'];
	}
	//edit yhy 2019/6/2
	if(!$flag && !empty($goods) && isset($res) && !empty($res) && $res['goods_use'] == 1){
		$intersect = array_intersect(array_column($goods,'goods_id'),explode(',',$res['use_goods_config']));
		//如果没有交集，那么就不可以使用该红包。清除掉使用红包的session
		if(empty($intersect)){
			$_SESSION['flow_order']['bonus'] = 0;
			$_SESSION['flow_order']['bonus_id'] = 0;
			$_SESSION['flow_order']['bonus_sn'] = '';
			return array();
		}else{
			//如果有交集，查询商品总额是否满足红包使用条件，不满足，清除掉红包
			$money = 0;
			foreach($goods as $v){
				if(in_array($v['goods_id'],$intersect)){
					$money += $v['formated_subtotal'];
				}
			}
			if($money<$res['min_goods_amount']){
				$_SESSION['flow_order']['bonus'] = 0;
				$_SESSION['flow_order']['bonus_id'] = 0;
				$_SESSION['flow_order']['bonus_sn'] = '';
				return array();
			}else{
				$res['type_money'] = min($res['type_money'],$money);
			}
		}
...
function get_total_bonus($supplier_money_info)
{
	...
	if(!is_array($supplier_money_info)){
		...
		$order_total = order_bonus_filter(0,$amount);
		...
		foreach($supplier_money_info as $key => $val){
			...
			$order_total += order_bonus_filter($key,$val);
...
/*查询订单可以获得的按订单金额发放的红包总额
**查询按订单金额发放的红包并进行过滤,按照是否根据区间金额发放，以及是否限制发放次数,并返回红包金额
** 如果是按照区间金额发放的红包，查询用户可领取最大数量
** 按照红包可发放会员级别进行过滤
**edit yhy 2019/5/15
*/
function order_bonus_filter($supplier_id,$order_amount){
	$day    = getdate();
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	$swhere = "";
	if($supplier_id){
		$swhere = " AND supplier_id = ".$supplier_id;
	}
	//查询按订单金额方法的红包
	$sql = "SELECT * FROM ". $GLOBALS['ecs']->table('bonus_type') ." WHERE send_type = '" . SEND_BY_ORDER . "' AND send_start_date <= '$today' AND send_end_date >= '$today' ".$swhere;
	$bonus_list = $GLOBALS['db']->getAll($sql);
	$money = 0;
	foreach($bonus_list as $k=>$v){
		if($v['user_rank_on']){//按照会员级别进行过滤，过滤掉当前会员等级不能获得的红包
			$user_rank_list = explode(',',$v['user_rank']);
			//查询会员级别
			$user_rank = intval($_SESSION['user_rank']);
			if(!in_array($user_rank,$user_rank_list)){
				continue;
			}
		}
		//按照固定价格发放
		if($v['money_type'] == 0){
			$money += FLOOR($order_amount/$v['min_amount'])*$v['type_money'];
		}else{
			if($v['user_bonus_max']>0){
				//查询用户已经获得红包的数量
				$bonus_sql = "SELECT COUNT(bonus_id) FROM ". $GLOBALS['ecs']->table('user_bonus') ." WHERE bonus_type_id=".$v['type_id']." AND user_id=". $_SESSION['user_id'];
				$bonus_count = $GLOBALS['db']->getOne($bonus_sql);
				if($bonus_count>=$v['user_bonus_max']){
					continue;
				}
			}
			//查询订单价格属于哪个区间
			$money_block = unserialize($v['money_block']);
			$left = array_column($money_block,'left');
			array_multisort($left,SORT_DESC,$money_block);
			
			//查询订单金额所属区间
			foreach($money_block as $block){
				if($order_amount>$block['left']){
					$money += ($block['money'] == -1) ? $order_amount : $block['money'];
					break;
				}
			}
		}
...
/**
 * 发红包：发货时发红包  //edit by yhy 支付成功后发放红包
 * @param   int     $order_id   订单号
 * @return  bool
 */
function send_order_bonus($order_id,$supplier_id)
{
	...
	//echo "<pre>";print_r($bonus_list);exit;
    /* 如果有红包，统计并发送 */
    if ($bonus_list)
    {
        /* 用户信息 */
        ...
        foreach ($bonus_list AS $bonus)
        {
            ...
            /* 修改用户红包 */
             $sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_bonus') . " (bonus_type_id, user_id, supplier_id,bonus_money) " ."VALUES('$bonus[type_id]', '$user[user_id]', '$supplier_id','$bonus[type_money]')";
...
{	
    ...
    $day    = getdate();
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    $sql = "SELECT b.type_id, b.type_money,user_bonus_max,send_type, SUM(o.goods_number) AS number " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS o, " .
                      $GLOBALS['ecs']->table('goods') . " AS g, " .
    ...
    /* 查询订单日期 */
    $sql = "SELECT add_time,user_id " .
            " FROM " . $GLOBALS['ecs']->table('order_info') .
            " WHERE order_id = '$order_id' LIMIT 1";
    $user_info = $GLOBALS['db']->getRow($sql);
	$order_time = $user_info['add_time'];
	$user_id = $user_info['user_id'];
	$order_bonus_list = get_order_bonus($amount,$user_id,$order_time,$supplier_id);
	//echo "<pre>";print_r($order_time);exit;
	//echo "<pre>";print_r($order_bonus_list);
    ///* 查询按订单发的红包 */
	 //$sql = "SELECT type_id, type_money,user_bonus_max,send_type, IFNULL(FLOOR('$amount' / min_amount), 1) AS number " .
            //"FROM " . $GLOBALS['ecs']->table('bonus_type') .
            //"WHERE send_type = '" . SEND_BY_ORDER . "' " .
			...
			//}
    $list = array_merge($list, $order_bonus_list);
    return array_values($list);
}

function get_user_rank_by_userid($user_id){
	$user_info = $GLOBALS['db']->getRow('SELECT user_rank,rank_points FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id = ' . $user_id);
	$user_rank = $user_info['user_rank']; 
	$rank_points = $user_info['rank_points']; 
	 $row = $GLOBALS['db']->getRow('SELECT discount, rank_id, special_rank FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_id = '$user_rank'");
    if (!empty($row)){
        if ($row['special_rank']){
            return $row;
        }else{
            return $GLOBALS['db']->getRow(
                'SELECT discount, rank_id FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE min_points >= '$rank_points'  AND special_rank=0 ORDER BY min_points ASC LIMIT 1"
            );
        }
    }else{
        return $GLOBALS['db']->getRow(
            'SELECT discount, rank_id FROM ' . $GLOBALS['ecs']->table('user_rank') . " WHERE '$rank_points' >= min_points AND special_rank=0 ORDER BY min_points DESC LIMIT 1"
        );
    }
}
...
** yhy 2019/5/15
*/
function get_order_bonus($order_amount,$user_id,$order_time,$supplier_id=0){
	 $user_rank = get_user_rank_by_userid($user_id);
	 ...
	 foreach($list as $k=>$v){
		 ...
		 if($v['user_rank_on']){
			$user_rank_list = explode(',',$v['user_rank']);
			 //echo "<pre>";print_r($user_rank_list);
			//查询会员级别
			if(!in_array($user_rank['rank_id'],$user_rank_list)){
				continue;
			}
		 }
		//按照固定价格发放
		if($v['money_type'] == 0){
			$bonus_list[] = array(
				...
				'user_bonus_max'=>$v['user_bonus_max'],
				'send_type'=>$v['send_type'],
				'number'=>FLOOR($order_amount/$v['min_amount'])
			);
		}else{
			if($v['user_bonus_max']>0){
				//查询用户已经获得红包的数量
				$bonus_sql = "SELECT COUNT(bonus_id) FROM ". $GLOBALS['ecs']->table('user_bonus') ." WHERE bonus_type_id=".$v['type_id']." AND user_id=". $user_id;
				$bonus_count = $GLOBALS['db']->getOne($bonus_sql);
				if($bonus_count>=$v['user_bonus_max']){
					continue;
				}
...
function cart_weight_price2($type = CART_GENERAL_GOODS, $supplier_id)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE extension_code = 'package_buy' AND (session_id = '" . SESS_ID . "' OR user_id=".$_SESSION['user_id'].")";
    
	if($_SESSION['sel_cartgoods']){
            $sql .= " AND rec_id in (".$_SESSION['sel_cartgoods'].") ";
      }
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c '.
                'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = c.goods_id '.
                "WHERE g.supplier_id='". $supplier_id ."' and (c.session_id = '" . SESS_ID . "' OR c.user_id=".$_SESSION['user_id'].") ".
                "AND rec_type = '$type' AND g.is_shipping = 0 AND c.extension_code != 'package_buy'";
	   if($_SESSION['sel_cartgoods']){
...
//生成商品订单
function creat_dzp_order($user_id,$goods_id,$consignee,$dzp_id)
{
    $sql="select * from".$GLOBALS['ecs']->table('goods') . "where goods_id = '$goods_id'";
    $goods_detail =  $GLOBALS['db']->getRow($sql);
    $order=array();
    ...
    $order['extension_code']='choujiang';


    $order['goods_amount']=$goods_detail['shop_price'];
    $order['shipping_fee']=0;
    $order['insure_fee']=0;
    $order['bonus']=0;
    $order['order_amount']=$goods_detail['shop_price'];
    $order['add_time']=time();
    $order['pay_time']=time();
    $order['is_separate']=1;
    $order['rebate_ispay']=1;
    $order['froms']="抽奖活动";
    ...
    $goods_value=$goods_detail;
    if(!empty($goods_value))
    {
        $sql = "INSERT INTO " .$GLOBALS['ecs']->table('order_goods'). " (dzp_id, order_id, goods_id, goods_name,goods_sn,goods_number,market_price,goods_price,is_real)" .
            "VALUES ('$dzp_id', '$new_order_id', '$goods_value[goods_id]', '$goods_value[goods_name]', '$goods_value[goods_sn]', '1', '$goods_value[market_price]', '$goods_value[shop_price]', '$val[is_real]')";
        $GLOBALS['db']->query($sql);
    }
    return $new_order_id;
}
