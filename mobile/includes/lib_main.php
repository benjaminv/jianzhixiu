...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_id '.
        ...
        " WHERE u.user_id = '$_SESSION[user_id]' and user_rank = '$fenxiao_rank_id'";
    $user_id_rank= $GLOBALS['db']->getOne($sql);
    if(!empty($user_id_rank))
    {
        $sql = "UPDATE " .$GLOBALS['ecs']->table('users'). " SET".
            " parent_id = 0 ".
            " WHERE user_id = '" . $_SESSION['user_id'] . "'";
    ...
    //注册后推送消息
    $gmtime=gmtime()-3600;
    $sql="select user_id,parent_id from".$GLOBALS['ecs']->table('users')."where user_id = '$_SESSION[user_id]' and join_status = 0 and '$gmtime' < reg_time ";
    $join_user= $GLOBALS['db']->getRow($sql);

    if(!empty($join_user)&&$join_user['parent_id']>0)
    {
        $sql = "UPDATE " .$GLOBALS['ecs']->table('users'). " SET".
            " join_status = 1 ".
            " WHERE user_id = '" . $_SESSION['user_id'] . "'";
        ...
        huiyuan_join($join_user['parent_id'],$join_user['user_id']);
    }
...
function get_user_info($id=0)
{
	...
	if(SSO_USERCENTER){
		...
		$cerditInfo = $GLOBALS['ssoObj']->creditGetByUserid($id);
...
function recalculate_price()
{
    ...
    $sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id, g.promote_price, g.promote_start_date, c.goods_number,'.
            ...
            "WHERE session_id = '" .SESS_ID. "' OR c.user_id='".$_SESSION['user_id']."' AND c.parent_id = 0 AND c.is_gift = 0 AND c.goods_id > 0 " .
...
//商品当月限购数量  
//限购判断，如果存在货品并且货品也限购的话，限购数量取货品的数量。否则取商品的限购数量
function xz_number($goods_id,$num,$goods_attr_id)
{
    /* 取得商品信息 */
    $sql = "SELECT g.xg_goods_number,g.goods_number".
        " FROM " .$GLOBALS['ecs']->table('goods'). " AS g ".
        " WHERE g.goods_id = '$goods_id'" .
        " AND g.is_delete = 0";
    $goods = $GLOBALS['db']->getRow($sql);
    if(!empty($goods_attr_id)){
        $spec = explode(",",$goods_attr_id);
        if (is_spec($spec))
        {
    ...
    $where_attr='1 ';
    if(empty($goods_attr_id)){
       // $where_attr.=" and og.goods_attr_id is null ";
    }
    else{
        $where_attr.=" and og.goods_attr_id = '$goods_attr_id' ";
    }
	
    //当月销量
    $sql = "SELECT IFNULL(sum(og.goods_number),0) as goods_attr_count
            FROM " .$GLOBALS['ecs']->table('order_goods'). " AS og
            LEFT JOIN " .$GLOBALS['ecs']->table('order_info'). " AS oi
                ON og.order_id = oi.order_id
            WHERE ".$where_attr." and  oi.user_id='$_SESSION[user_id]' and og.goods_id = '$goods_id' and oi.add_time > '$start_time' and oi.add_time < '$end_time'";
    $goods_attr_count = $GLOBALS['db']->GetOne($sql);
    if (empty($goods_attr_id))
    {
        /* 取规格的货品库存 */
        if ($num+$goods_attr_count > $goods['xg_goods_number'])
        {
            //$GLOBALS['err']->add("购买数量已经超过本月限购！", ERR_OUT_OF_STOCK);

            return false;
        }
    }
    else{
        if ($num+$goods_attr_count > $product_info['xg_goods_number'])
        {
            //$GLOBALS['err']->add("购买数量已经超过本月限购！", ERR_OUT_OF_STOCK);
