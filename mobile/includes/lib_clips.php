...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
	...
	if(isset($_SESSION['user_rank'])) $user_rank = $_SESSION['user_rank'];
 ...
 * @return  int
 */
function insert_user_account_new($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
        ' (user_id,acc_type, alipay_cn ,alipay_name , admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid)'.
        " VALUES ('$surplus[user_id]', '$surplus[acc_type]', '$surplus[alipay_cn]', '$surplus[alipay_name]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0)";
    $GLOBALS['db']->query($sql);
...
function get_online_payment_list($include_balance = true)
{
    ...
    if (!$include_balance && strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger'))
    {
        ...
        $sql .= " AND (pay_code = 'alipay' or pay_code= 'unionpay' or pay_code= 'wxpay_h5') ";
    }
