...
//获取可以使用红包抵扣的商品列表 yhy 2019/5/16
function get_bonus_goods($type_id=0,$limit = 0){
	
	
	$day    = getdate();
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	//$sql = "SELECT use_goods_config FROM ".$GLOBALS['ecs']->table('bonus_type')." WHERE goods_use=1 AND send_start_date<= '$today' AND send_end_date >='$today'";

	$sql = "SELECT use_goods_config,goods_use FROM ".$GLOBALS['ecs']->table('bonus_type')." WHERE use_start_date<= '$today' AND use_end_date >='$today'";
	if($type_id){
		$sql .= " AND type_id=".$type_id; 
	}
	$res = $GLOBALS['db']->getAll($sql);
	$all_goods_sql = "SELECT goods_id FROM ".$GLOBALS['ecs']->table('goods')." WHERE is_delete=0 AND is_on_sale=1";
	$all_goods = $GLOBALS['db']->getAll($all_goods_sql);
	$all_goods_ids = array_column($all_goods,'goods_id');
	$use_goods_config = array();
	foreach($res as $v){
		if($v['goods_use'] == 0){
			$use_goods_config =array_merge($use_goods_config,$all_goods_ids);
		}else{
			$use_goods_config =array_merge($use_goods_config,explode(',',$v['use_goods_config']));
		}
	}
	array_unique($use_goods_config);
	if($use_goods_config){
		$goods_sql = "SELECT goods_id,goods_name,shop_price,market_price,promote_price,goods_thumb FROM ".$GLOBALS['ecs']->table('goods')." WHERE is_delete=0 AND is_on_sale=1 AND goods_id IN (".implode(',',$use_goods_config).") ORDER BY sort_order DESC";
		if($limit>0){
			$goods_sql.=" LIMIT 0,".$limit;	
		}
		$list = $GLOBALS['db']->getAll($goods_sql);
		foreach($list as $idx=>$row){
			$list[$idx]['final_price'] = price_format(get_final_price($row['goods_id'], 1, false));
            $list[$idx]['thumb']        = get_pc_url().'/'. get_image_path($row['goods_id'], $row['goods_thumb'], true);
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        /* 秒杀倒计时  月梦网络 */
        $time = gmtime();
		//查询该商品是否对应着有正在进行的秒杀活动
		$sql = "select s.* from ". $GLOBALS['ecs']->table('goods') . " g"." LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " s ON g.goods_id = s.goods_id"." where g.goods_id = $goods_id and s.is_seckill=1";	
		$seckill_info = $GLOBALS['db']->getRow($sql);
        if ($time >= $seckill_info['seckill_start_date'] && $time <= $seckill_info['seckill_end_date'] && $seckill_info['is_seckill'] == 1)
        {
			
            //获取之前的秒杀订单,判断是否过期,过期则取消订单。
            $sql = "select * from ".$GLOBALS['ecs']->table("order_info")."where extension_code = 150 and pay_status = 0 and order_status !=2";
            $seckill_order = $GLOBALS['db']->getAll($sql);
            foreach ($seckill_order as $key=>$value){
                if($time>$value['add_time']+60*10){
                    //秒杀订单10分钟未付款，则取消该订单
                    $sql = "update ".$GLOBALS['ecs']->table("order_info")."set order_status = 2 where order_id = $value[order_id]";
                    $GLOBALS['db']->query($sql);
                    //将秒杀数量返还
                    $sql = 'select * from '.$GLOBALS['ecs']->table("order_goods")."where order_id = $value[order_id]";
                    $seckill_goods = $GLOBALS['db']->getAll($sql);
                    foreach ($seckill_goods as $k=>$v){
						
                        //$sql = "select * from ".$GLOBALS['ecs']->table("goods")."where goods_id = $v[goods_id]";
						$sql = "select s.* from ". $GLOBALS['ecs']->table('goods') . " g"." LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " s ON g.goods_id = s.goods_id"." where g.goods_id = $v[goods_id]";						
                        $seckill_data = $GLOBALS['db']->getRow($sql);
                        if($time >= $seckill_data['seckill_start_date'] && $time <= $seckill_data['seckill_end_date'] && $seckill_data['is_seckill'] == 1){
							
                            $sql = "update ".$GLOBALS['ecs']->table("seckill_goods")."set seckill_num = seckill_num+$v[goods_number] where goods_id = $v[goods_id] and is_seckill=1";
                            $GLOBALS['db']->query($sql);
                        }
                    }
                }
            }
            $row['gmt_end_time']  = $seckill_info['seckill_end_date'];
            $row['pd_seckill'] = 1;
			$row['seckill_num'] = $seckill_info['seckill_num'];
			//设置购物流程为
			//$_SESSION['extension_id'] = $seckill_info['seckill_id'];
        }
        else
        {
            /* 促销时间倒计时 */
            $time = gmtime();
            if ($time >= $row['promote_start_date'] && $time <= $row['promote_end_date'] && $row['is_promote'] ==1 )
            {
            ...
            {
                //判断是不是秒杀商品,还未开始的状态
				echo $seckill_info['seckill_start_date'];
                if($time < $seckill_info['seckill_start_date']  && $seckill_info['is_seckill'] == 1){
                    $row['gmt_end_time'] = $seckill_info['seckill_start_date'];
                    $row['pd_seckill'] = 3;
                }else{
                    $row['gmt_end_time'] = 0;
                }
