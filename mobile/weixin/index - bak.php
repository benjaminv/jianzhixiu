...
if(!$followInfo or $followInfo['expire_in']-86400<time()){
	...
	if($info) $xxx= $api->followUser($wxid,$info);
}
	/*

// 调试 
dm299_log($wxid);
dm299_log($weixin->errCode);
dm299_log($weixin->errMsg);
dm299_log($info);
dm299_log($xxx);

*/
//echo $weixin->text('111')->reply();
	//exit; 
...
if ($event['event'] == "SCAN"){
	...
	if($api->scanLogin($content,$wxid) === true){
		echo $weixin->text("您使用扫一扫功能登陆网站成功！如网页没有跳转请点击底部按钮，谢谢！")->reply();
					...
					if($ticket){
						...
						if(false !==$local_file){	
							 ...
							 fclose($h_local_file);
								...
								$text = "保存二维码图片的路径mobile/weixin/images/qrcode或mobile/weixin/images/head则没可写权限，请修改！";
								echo $weixin->text($text)->reply();exit;
						}
					}else{
...
// 月梦网络 Added by PRINCE QQ 124861234 2016年7月18日
...
// 调试 http://jd1.dm299.com/mobile/data/log.txt
function dm299_log($arg) {
	$str = "\r\n-- ". date('Y-m-d H:i:s'). " --------------------------------------------------------------\r\n";
	if (is_array($arg))
	{
		$str .= '$arg = array(';
	...
	file_put_contents(ROOT_PATH . DATA_DIR . '/log.txt', $str,FILE_APPEND);
}
