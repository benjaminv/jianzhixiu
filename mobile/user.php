...
if(empty($_SESSION['user_id']))
{
	if(! in_array($action, $not_login_arr))
	{
		if(in_array($action, $ui_arr))
		{
			...
			{
				$back_act = "https://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?' . strip_tags($_SERVER['QUERY_STRING']);
			}
			$action = 'login';
			if(empty($back_act) && isset($GLOBALS['_SERVER']['HTTP_REFERER'])){
				$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? "https://".$_SERVER['HTTP_HOST'].'/index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
			}else{
				 if(isset($_REQUEST['jump']) && trim($_REQUEST['jump'])=='jszx'){
					$back_act = "https://".$_SERVER['HTTP_HOST'].'/moblie/flow.php?step=checkout';
				 }else{
					$back_act = "https://".$_SERVER['HTTP_HOST'].'/mobile/user.php';
				 }
			}
			//header("location: ".OSS_LOGIN."/?dest=".$back_act);exit;
		}else{
			// 未登录提交数据。非正常途径提交数据！
...
function action_act_attestation()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    {
        show_message("请上传身份证正面！", '返回', '', 'wrong');
    }
    if(empty($card_num))
    {
        show_message("请填写正确的身份证号码！", '返回', '', 'wrong');
    }
    //判断上传的文件是否出错,是的话，返回错误
    if($_FILES["file"]["error"])
    {
        show_message($_FILES["file"]["error"], '返回', '', 'wrong');
    }
    else
    {
        //没有出错
        ...
        {
            //防止文件名重复
            $filename ="images/users/".time().$_FILES["file"]["name"];
            //转码，把utf-8转成gb2312,返回转换后的字符串， 或者在失败时返回 FALSE。
            $filename =iconv("UTF-8","gb2312",$filename);
            //检查文件或目录是否存在
            if(file_exists($filename))
            {
                show_message("该文件已存在", '返回', '', 'wrong');
            }
            else
            {
                //保存文件,   move_uploaded_file 将上传的文件移动到新位置
                move_uploaded_file($_FILES["file"]["tmp_name"],ROOT_PATH.$filename);//将临时地址移动到指定地址
                $file_img="http://".$_SERVER['SERVER_NAME']."/mobile/".$filename;
                $resurt=Identity_Card($file_img);
                if($resurt['code']==1)
                {
                    $real_name=$resurt['result']['name'];
                    $card = $resurt['result']['code'];
                    $face_card="/mobile/".$filename;

                    $sql = 'UPDATE ' . $ecs->table('users') . " SET real_name = '$real_name',card = '$card',face_card = '$face_card',card_status = 1 WHERE user_id = '$_SESSION[user_id]'";
                    $db->query($sql);
                }
                else
                {
                    show_message($resurt['msg'], '返回', '', 'wrong');
                }
            }
        }
        else
        {
            show_message("文件类型不对", '返回', '', 'wrong');
        }
    }
    show_message("认证成功！", '返回个人中心', 'user.php', 'info');
				...
				if($tag == 1) //第一次注册绑定上级分销商
				{
					...
					if($parent_id)
					{
						...
						$GLOBALS['db']->query("DELETE FROM " . 
								$GLOBALS['ecs']->table('bind_record') . 
								" WHERE wxid = '" . $_SESSION['wxid'] . "'");
                        require_once(ROOT_PATH  . 'mobile/weixin/weixin_notice.php');
                        huiyuan_join($_SESSION['user_id'],$parent_id);
					}
				}
			}
		}
...
function action_other_login()
{
	...
	$redirect_url = "https://" . $_SERVER["HTTP_HOST"] . str_replace("user.php", "index.php", $_SERVER["REQUEST_URI"]);
...
function action_login()
{
	...
	if(isset($_REQUEST['teshu_tiao_zhuan']) && $_REQUEST['teshu_tiao_zhuan'] == 'tiaozhaunhuilai'){

		$back_act ='/mobile/sao_ma.php?&bianhao='.$_REQUEST['bianhao'].'&jifen='.$_REQUEST['jifen'];
...
{
	//header("location: ".OSS_LOGIN."/logouting");
	//exit;
	...
	{
		$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? "https://".$_SERVER['HTTP_HOST'].'/mobile/index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
	}
		...
		if($_FILES['headimg']['size'] > 2048000)
		{
		 ...
		 show_message("头像文件过大", $_LANG['profile_lnk'], 'user.php?act=profile', 'info');
			...
			//把上传的临时文件移动到up目录下面 
	           ...
	           $sql = 'UPDATE ' . $ecs->table('users') . " SET `headimg`='".$adname."'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
...
function action_del_back_order ()
{
	...
	{
		$backsql = "select back_id from " . $ecs->table('back_order') . " where back_id='$back_id' AND back_type=1 ";
		$back_info = $db->getAll($backsql);
		...
		$db->query($sql);
		if(ODOO_ERP && !empty($back_info)){
			//同步删除erp中的退货单退款单
			$odooErpObj = OdooErp::getInstance();
			$odooErpObj->syncRefundCancel($back_info);
		}
...
function action_back_order_detail_edit ()
{
	...
	$db->query($sql);
		...
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			//edit yhy 更新退货单的物流单号 
			$res = $odooErpObj->syncRefundOrderByOrdersn($back_id,1,1);
		}
...

{
   ...
   $goods_sql = $goods_where = "";
   if($goods_id){
	$goods_sql = " LEFT JOIN ". $ecs->table('back_goods') ." b ON a.back_id=b.back_id ";
	$goods_where = " AND b.goods_id= ".$goods_id." AND b.product_id=".$product_id;
   }
   $sql = "select a.back_id from " . $ecs->table('back_order') . " a ".$goods_sql." where a.order_id='$rec_id' ".$goods_where;
   ...
   if($goods_id){
	   //订单商品信息
		$goods = $db->getAll("SELECT o.*,g.goods_name,g.goods_thumb FROM " . $ecs->table('order_goods') . " as o left join ".$ecs->table('goods')." as g on o.goods_id = g.goods_id WHERE order_id = '$rec_id' and o.goods_id='$goods_id' AND o.product_id = ".$product_id);
		$order_goods=$db->getRow("select goods_number,goods_price from". $ecs->table('order_goods') ."where order_id='$rec_id' and goods_id='$goods_id' AND product_id='$product_id'");
   }else{
...
function action_back_order_act ()
{
	...
	if (!$_POST['order_all'])
	{
		...
		$goods_attr =  ! empty($_POST['goods_attr']) ? trim($_POST['goods_attr']) : "";
		$tui_goods_number = $_POST['tui_goods_number'] ? intval($_POST['tui_goods_number']) : 1;
		$tui_goods_price = $_POST['tui_goods_price'] ? floatval($_POST['tui_goods_price']) : 0;
	}
	...
	$back_type = intval($_POST['back_type']);//4为退款  1为退货退款   yhy 
	...
	if($_POST['order_all']){
		...
		//退货退款的话，按照退货退款的商品和货品id去查询是否申请过
		if(empty($goods_id)){
			show_message('没有可以退款的商品！');
			exit();
		}else{
			$sql = "SELECT COUNT(a.order_sn) FROM ".$ecs->table('back_order')." a LEFT JOIN ".$ecs->table('back_goods')." b ON a.back_id = b.back_id WHERE a.order_id='$order_id' AND b.goods_id = '$goods_id' AND b.product_id= ".$product_id;
		}
	}
	...
	if($db->getOne($sql)==0){
		...
		{//退货退款
			...
			$have_tuikuan = 1;
			$order_goods_sql = "SELECT use_points_by_one,deduction_amount_by_one FROM " . $ecs->table('order_goods') . " WHERE order_id ='" . $order_id ."' AND goods_id = '" . $goods_id ."' AND product_id=".$product_id;
			$tui_goods_back_type1_info = $db->getRow($order_goods_sql);

			$sql =  "insert into " . $ecs->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, back_goods_number, back_goods_price, status_back, use_points_by_one, deduction_amount_by_one ) values('$back_id','$goods_id','$goods_name','$goods_sn','$product_id','$goods_attr',0,'$tui_goods_number','$v[goods_price]',5,'$tui_goods_back_type1_info[use_points_by_one]','$tui_goods_back_type1_info[deduction_amount_by_one]')";
		...
		{//退款
			...
			foreach($order_info['goods_list'] as $goods_info)
			{
				...
				$sql = "INSERT INTO " . $GLOBALS['ecs']->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, " . "back_goods_number, back_goods_price, status_back, use_points_by_one, deduction_amount_by_one) " . " values('$back_id', '".$goods_info['goods_id']."', '".$goods_info['goods_name']."', '".$goods_info['goods_sn']."', '".$goods_info['product_id']."', '".$goods_info['goods_attr']."', '4', '".$goods_info['goods_number']."', '".$goods_info['goods_price']."', '5'," . "'" . $goods_info['use_points_by_one'] ."'".  ",'" . $goods_info['deduction_amount_by_one']."') ";
				$db->query($sql);
			}
	...
	if($have_tuikuan)
	{
		...
		{
			$price_refund = $tui_goods_number*$tui_goods_price;
		}
	...
	$ecs = $GLOBALS['ecs'];
        ...
        if($db->query("UPDATE " . $ecs->table('users') . " set address_id = $address_id  WHERE user_id='$user_id'")){ 
			if(ODOO_ERP){
				$odooErpObj = OdooErp::getInstance();
				//edit yhy 同步erp中的会员
				$res = $odooErpObj->syncAddressByAddressid($address_id);	
			}
...
function action_drop_consignee()
{
	...
	if(drop_consignee($consignee_id))
	{
		if(ODOO_ERP){
			$odooErpObj = OdooErp::getInstance();
			//edit yhy 同步erp中的会员
			$res = $odooErpObj->syncAddressUnlinkByAddressid($consignee_id);	
		}
...
function action_affirm_received()
{
	...
	{
		$sql = "SELECT order_id, supplier_id FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
		...
		//yhy edit 2019/5/17确认收货的时候发放订单红包 
		//2019/5/18更改为支付成功后发放
		//send_order_bonus($order['order_id'],$order['supplier_id']);
		ecs_header("Location: user.php?act=order_list\n");
		exit();
	}
	else
...
/*会员积分明细界面*/
function action_integral_detail()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    // 模板赋值
    $smarty->assign('points_amount', price_format($points_amount, false));
...
function action_act_account()
{
		...
		if($amount < $deposit_least_money)
	{
        ...
        $surplus['acc_type'] = isset($_POST['acc_type']) ? floatval($_POST['acc_type']) : 0;
        $surplus['alipay_cn'] = ($_POST['alipay_cn']);
        $surplus['alipay_name'] = ($_POST['alipay_name']);
		// 插入会员账目明细
		$amount = '-' . $amount;
		$surplus['payment'] = '';
		$surplus['rec_id'] = insert_user_account_new($surplus, $amount);
	...
	else
	{
		...
		if($payment_info['pay_name'] == '支付宝'){
			$payment_info['pay_button'] = ' <a href="./pay/alipayapi.php?out_trade_no='.$order['log_id'].'&total_fee='.$order['order_amount'].'"  class="bnt_1">去支付宝支付</a>';
		}
		if($payment_info['pay_name'] == '微信支付'){
			$payment_info['pay_button'] = ' <a href="weixindeposit.php?out_trade_no='.$order['log_id'].'"  class="bnt_1">微支付</a>';
		}
...
function action_affiliate()
{
	...
	else
	{
		...
		$shopurl = $ecs->url();
		$goods['goods_img'] = (strpos($goods['goods_img'], 'https://') === false && strpos($goods['goods_img'], 'https://') === false) ? $shopurl . $goods['goods_img'] : $goods['goods_img'];
		$goods['goods_thumb'] = (strpos($goods['goods_thumb'], 'https://') === false && strpos($goods['goods_thumb'], 'https://') === false) ? $shopurl . $goods['goods_thumb'] : $goods['goods_thumb'];
...
function get_user_backorders ($user_id, $num = 10, $start = 0)
{
	...
	while($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$row['refund_money_1'] = price_format($row['refund_money_1'] - $goods_points_deduction_amount, false);
...
function Identity_Card($image_src)
{
    $host = "https://ocridcard.market.alicloudapi.com";
    $path = "/idimages";
    $method = "POST";
    $appcode = "8327531aeffd4a6298376a1b236da85d";
    $headers = array();
    array_push($headers, "Authorization:APPCODE " . $appcode);
    //根据API的要求，定义相对应的Content-Type
    array_push($headers, "Content-Type".":"."application/x-www-form-urlencoded; charset=UTF-8");
    $querys = "";
    //$bodys = 'image=http://img3.fegine.com/image/idcard.jpg'.'&idCardSide=front'; //图片 + 正反面参数 默认正面，背面请传back
    $bodys = 'image='.$image_src.'&idCardSide=front'; //图片 + 正反面参数 默认正面，背面请传back
    //或者base64
    ...
    $out_put_html=json_decode($out_put,true);
    return $out_put_html;
}
?>
