...
if (!$smarty->is_cached('tuiguang.dwt', $cache_id))
{
	...
	if(!empty($user_id)){
		...
		$ret = $GLOBALS['db']->getRow($query_sql);
		$fenxiao_sql = "SELECT is_fenxiao FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
		$isfenxiao =  $GLOBALS['db']->getOne($fenxiao_sql);
	}else{
   ...
   $smarty->assign('ret',  $ret);
	...
	if($isfenxiao > 0){
		$guide="您现在是东家,赶快去分享吧！";
		$smarty->assign('isfenxiao', 'ok');
	}else{
		$guide="您还不是东家,请购买成为东家";
		$smarty->assign('isfenxiao', 'goumai');
	}
