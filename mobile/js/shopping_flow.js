...
function compute_points(surplus, points)
{
    Ajax.call('flow.php?step=compute_points', 'surplus=' + surplus + '&points=' + points,  compute_points_response, 'GET', 'JSON');
}

function compute_points_response(obj)
{
    document.getElementById('ECS_SURPLUS').value = obj.surplus;
    try {
        if(obj.hide_pay_div){
...
function checkOrderForm(frm)
{
  ...
  // 检查是否选择了支付配送方式
    ...
    if (parseFloat($("#temp_amount").val()) <= 0) {
        paymentSelected = true;
    }
  ...
  if (document.getElementById("ECS_INTEGRAL"))
  {
    ...
    if ($("#ECS_USE_POINTS").is(":checked")) {
        $("#ECS_USE_POINTS").val('1');
    } else {
        $("#ECS_USE_POINTS").val('0');
    }
